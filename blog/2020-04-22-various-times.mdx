...
Lately, as our receipt count has improved, we've been seeing long block times again. We have spent a
couple of days doing some performance analysis and instrumenting the consensus code a bit better,
and have come up with some changes that should hopefully improve the situation. This release mostly
focuses on some correctness issues with the signature phase of our HBBFT system. There will be
another release in a few days which should contain some speedups to HBBFT primitives, once we've had
a chance to validate them for a bit longer.
