...
On 4/18, a new Config Service release was deployed as part of the Oracle 1.0.0 release, which
...
- On 4/18, the outage was triggered by the protobuf mismatch between the Packet Router and Config
  Service. An emergency Packet Router update 0.0.22 was deployed to reconcile the protobuf mismatch.
- On 4/19, the outage was triggered by the protobuf mismatch between the Helium Router and Config
  Service, which caused the Helium Router hotspot location request rpc to Config Service to fail. As
  a result, devices cannot join since DevAddr allocation is based on hotspot location, and it
  created load & memory issue that leaked into other processes, which also led to degraded uplink &
...
- Second, make sure alerts are in place. In the 4/19 incident, if alerts were more exhaustive, our
  response time could have been faster. Specifically, we have grafana alerts for when join & uplink
  rate approaching 0. However, turns out grafana could categorize 0 as NONE, hence requiring a
  separate alarm that monitors for that state.
