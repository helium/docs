...
- Everyone who is placed on the deny list submitted removal requests, sometimes with multiple
  submissions per hotspot.
...
The new deny list detection criteria is based
on \***\*\*\*\*\***\*\*\***\*\*\*\*\***classifiers\***\*\*\*\*\***\*\*\***\*\*\*\*\*** which have
...
- The classifiers are run every 7 days to produce a new deny list and may include improvements to
  ...
  positives.
- The current, legacy, deny list is completely removed and a first deny list will be generated by
  the initial set of classifiers. The target date for this is **August 21, 2023**
...
is an example of the terrain intersection for a hotspot with abnormal behaviour, and one which is
correctly asserted:
...
witnesses. This measurement is called the Jaccard index. We also distinguish the analysis of inbound
vs outbound witnesses for the hotspot. It works by counting the total number of witnesses between
two hotspots and dividing by the number which is shared. Here is an example of this calculation for
a normal hotspot:
