...
This post-mortem describes the events leading to an incorrect A portion of the Utility Score for the
MOBILE SubNetwork, what happened, and what the team is doing moving forward.

<!--truncate-->

## Timeline

On July 21st, 2023, MOBILE Subscriber Mapping went live. This introduced a new rewardable entity
into the MOBILE ecosystem.

Helium Oracles are powered by a data pipeline that runs over a Proof of Coverage and network usage
...
The immediate fix for this was to correct the active device query to filter out MOBILE subscribers.
This was deployed shortly after we recognized the error.

**HIP 88**

Due to the implementation of HIP-88, tracking onboarding fees per device, this problem can no longer
happen again. Instead, a new oracle sets each device on-chain as active or inactive based off of the
