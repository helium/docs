...
Spectrum Access System (SAS) to operate. Please be sure to evaluate your site location and ability
to install these Small Cell Radios, and consider hiring a professional to ensure your installation
...
The [Helium Explorer](https://explorer.helium.com/) will include an additional tab for all 5G
Hotspots with active Small Cell Radios, "5G Statistics"

This tab includes `Small Cell Status` and `Small Cell Location` widgets to describe the active
status and Small Cell Radio Type respectively.

Small Cell Status will be `Active` if the Small Cell has broadcast a `heartbeat` to the SAS in the
past 10 minutes. Small Cell Location will be either `Indoor` or `Outdoor` depending on the Small
Cell Radio type.

:::note 5G Hotspots with more than one Small Cell Radio

Small Cell Status will reflect all attached Small Cell Radios in aggregate, if any connected Small
Cell Radio has broadcast a `heartbeat` to the SAS, the Status will be `Active` regardless of the
actual status of any other connected Small Cell Radio.

Refer to the 5G Hotspot dashboard for information on each connected Small Cell Radio (local network
connection only) [http://freedomfi-gateway.local/](http://freedomfi-gateway.local/)
...
The Spectrum Access status will be in one of three states. It is not uncommon to see this status
change frequently.
...
<div class="container" style={{ padding: 0, paddingTop: 0 }}>
  <div class="row">
    ...
    <div class="col col--3" style={{ marginBottom: 0 }}>
      ...
      <p>
        The Small Cell Radio Spectrum Access was recently <code>None</code> and is comming back
        online, this status should be transitory.
      </p>
    </div>
    <div class="col col--3" style={{ marginBottom: 0 }}>
      ...
      <p>
        See <a href="./cbrs-radios#spectrum-access-none-status">below</a> for more information.
      </p>
    </div>
    <div class="col col--3" style={{ marginBottom: 0 }}>
      ...
      <p>The CBRS Spectrum access service is disabled to the Small Cell Radio. </p>
      <p>
        See <a href="./cbrs-radios#spectrum-access-failure-status">below</a> for more information.
      </p>
    </div>
  </div>
</div>
...
There is nothing that you need or can do in these scenarios. The SAS will automatically re-grant
Spectrum Access once the DPA event has concluded.
...
DPA regions can be viewed in the map below. Remember, these coordinates are not exact, and the
effected regions for each DPA Event can reach as far as 150km inland.
...
For newly installed Small Cell Radios, this "Failure" status is likely because the registration
process has not been completed and submitted to the SAS.

You may see information similar to this:

> SAS status code: 200 SAS response message: Response data: installationParam

For any other warning messages, contact [FreedomFi Support](https://freedomfi.com/contact-us/)
directly.
