...
- **Stage 1: ** Genesis Rewards
  - _Current Implementation_:
    [Speed Test and Quality of Service](/5g-on-helium/mobile-poc#speed-tests-and-quality-of-service)
  - _Work in Progress_:
    [Heartbeats and Consistency of Service](/5g-on-helium/mobile-poc#heartbeats-and-reliability-of-service)
- **Stage 2: ** [5G Mappers Proof of Coverage](/5g-on-helium/mobile-poc#mobile-mappers)
- **Stage 3: ** [Additional Methods of Proof of Coverage](/5g-on-helium/mobile-poc#future-roadmap)

---

## PoC Reward Elegibility
...
To qualify for Genesis Rewards in a Reward Period, a CBRS Small Cell Radio must:
...
- Meet Speed Test Metrics of at least `Poor` on all metrics.
...
> - The reward Period is approximately 24 hours, with Reward submission occurring at approximately
>   01:30 UTC.
> - 5G Hotspots perform periodic Speed Tests which will be considered in Reward calculations.
> - A minimum Download speed, minimum Upload speed, and maximum Latency will be required by each 5G
>   Hotspot to ensure it is providing quality coverage.
...
than 6 most recent Speed Test results for each download, upload, and latency. This means that when
more than 6 Speed Test results are available, results that came before the 6th most recent test
won't be considered in the calculation. Additionally, the Moving Average is reset following a Speed
Test gap of `>12 hours` and a new series of Speed Test results must be collected before a Moving
...
This 5G Hotspot **will not** be eligible for rewards during the Reward Periods for which the results
of Tests 5 through 6 is the most recent as there is not enough data to calculate a Moving Average.
...
> Small Cell Radios must generate at least 1 "heartbeat" per 1 hour for any 12 hours in the 24-hour
> Reward Period.

At this Milestone, to qualify for the Rewards in a Reward Period, a CBRS Small Cell Radio must:
