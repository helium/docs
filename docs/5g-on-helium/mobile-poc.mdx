...
Helium Network with the Helium MOBILE Token.
...
During [Genesis](./mobile-poc#mobile-genesis), 5G Hotspots will begin to earn MOBILE by reporting
their status via a “heartbeat,” an automatic reporting process to the Helium Blockchain that the
Small Cell Radio is online and registered with the SAS.
...
Soon 5G Hotspots will perform Speed Tests that impact
[Rewards Scaling Multipliers](./mobile-poc#speed-test-tier-reward-scaling) in addition to Small Cell
Type.

When Speed Tests are included in Rewards calculations, the Reward weights will be as follows:

| Speed Test           | Acceptable Performance | Degraded Performance | Poor Performance |  Fail   |
| -------------------- | :--------------------: | :------------------: | :--------------: | :-----: |
...
the results of the Moving Average for each metric. See Examples of possible results below.**

:::

Speed Test metrics will be viewable on the 5G Hotspot's page on Explorer under the `5G Statistics`
tab. Here are some examples of each status and a brief description of each metric and the resulting
