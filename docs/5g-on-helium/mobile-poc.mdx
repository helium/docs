...
to the IOT Network. In the IOT Network, hexes determine the locations and number of Hotspots within
the given hex. In the MOBILE Network, hexes determine where the coverage exists. Modeled Coverage
data allows us to un-tie a Radio's location from its signal strength and propagation.

While all current 5G Hotspot models _also_ participate in the IOT Network and PoC, the CBRS Small
