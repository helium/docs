...
Returns the total rewards for a given account in a given timeframe in `bones`. Timestamps can be
given in ISO 8601 format (e.g. `2020-08-27T00:00:00Z`) or in relative time (e.g. `-1 week`, which
when url esacped becomes `-1%20week`). When ommitted the current time is assumed.
