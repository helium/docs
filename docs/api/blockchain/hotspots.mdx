...
Fetch the hotspots which are within a given number of meters from the given `lat` and `lon` coordinates. This
...
:::caution
The `/activity` route will be deprecated on May 1, 2022 and will be replaced by [`/roles`, described below](#hotspot-roles)
:::
...
This lists the activity for a Hotspot which only had two
transactions at the time of the request.
...
| min_time (required) | _string_ | First timestamp to include rewards for |
| bucket (optional) | _string_ | Get Data in Special Bucket ( hour / month / week ) |
