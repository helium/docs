...
To return data quickly and reduce the load on the API, a `cursor` may be returned from an API
request. The documentation for each route will indicate whether the result is paged with a cursor.

To request the next page of results, add a cursor URL parameter to the URL that was used to make the
first request.
...
Which will return the next page of results and the next `cursor` for the following page. A cursor
will be returned with each response until the last page is reached, at which point no cursor is
returned.

:::info

To improve caching the API may return an empty data array and a cursor. Use the cursor to request
more data.
...
Unless otherwise specified, API calls return values in the following units:

| value        | Units        | Conversion to HNT             |
| ------------ | ------------ | ----------------------------- |
| `amount`     | bones        | `100,000,000 bone` to `1 HNT` |
| `balance`    | bones        | `100,000,000 bone` to `1 HNT` |
| `stake`      | bones        | `100,000,000 bone` to `1 HNT` |
| `sum`        | bones        | `100,000,000 bone` to `1 HNT` |
| `fee`        | Data Credits | `100,000 DC` to `1 HNT`       |
| `dc_balance` | Data Credits | `100,000 DC` to `1 HNT`       |
