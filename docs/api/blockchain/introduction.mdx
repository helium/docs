...
To access the helium api via script you will need to specify a valid
`User-Agent` HTTP header field identifying your agent, all requests made without
a `User-Agent` header will result in an error `429 Slow down gun powder!`.
...
In order to request the next page of results, add a cursor URL parameter to the
URL that was used to make the first request.
...
In order to improve caching the API may return an empty data array and a cursor.
