...
These docs reflect the Validator Mainnet API. If you would like to use the
Testnet API, please use the following base url:
...
List known validators as registered on the blockchain. The results are paged. If
a `cursor` field is present in the response, more results are available.
...
```json
{
  "data": {
    ...
    "status": {
      ...
      "online": "online",
      "listen_addrs": [
        "/ip4/690.0.0.180/tcp/2154",
        "/ip4/340.210.76.150/tcp/2154"
      ],
      "height": 993737
    },
    "stake_status": "staked",
...
Fetch the validators which map to the given 3-word animal name. The name must be
...
replaced by [`/roles`, described below](#validator-roles) :::
...
Returns the list of validators that are currently elected to the consensus
group.
...
Returns the list of validators that were in the consensus group at a given block
height
