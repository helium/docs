...
### Create a Custom HTTP Integration (Staging)

```
POST https://staging-console.helium.wtf/api/v1/integrations
```
...
| headers (optional) | _string_ | Map of header keys and values { “content-type”: “application/json” } |
...
### Create a Custom MQTT Integration (Staging)
...
### Create a Custom Azure IoT Hub Integration (staging)
...
| ------------------ | -------- | ------------------------------------ |
| name (required) | _string_ | A unique human-friendly name          |
| type (required) | _string_ | “azure_hub”             |
| azure_hub_policy_name (required) | _string_ | Azure IoT Hub policy name            |
| azure_hub_name (required) (required)    | _string_ | Azure IoT Hub name |
| azure_hub_primary_key (required) | _string_ | Azure IoT Hub primary key |
...
### Create a Custom AWS IoT Core Integration (staging)
