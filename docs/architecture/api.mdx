...
The Hotspot API is provided free of charge by the Helium Foundation. The API runs inside an AWS EKS
cluster and leverages an RDS instance for improved accessibility of on-chain Hotspot data. The API
sits behind Cloudflare to offer reduced latency via endpoint caching. As on-chain Hotspot data is
updated (e.g., attributes such as location, activity, and ownership), invalidations are created to
ensure up-to-date data is provided to API consumers.

:::note

The latitude/longitude data provided by the API is obfuscated to hex res8 rather than res12 stored 
on-chain on Solana.
