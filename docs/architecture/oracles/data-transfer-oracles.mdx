...
out [this tutorial](/iot/run-an-lns) to learn more about how to run your own LoRaWAN Network Server.
...
ingested by the Reward Oracles so that the hotspots delivering that packet are duly rewarded.
...
- Confirm that each OUI and the associated LNS have enough DC to pay for the packets received.
  - For every Packet Report received, if the DC balance for an OUI has fallen below 3.5M DC, the
    route will be locked.
  - The Packet Verifier checks the DC balance of every locked OUI every 30 minutes, and unlocks them
    if their DC balance is above 3.5M DC.
