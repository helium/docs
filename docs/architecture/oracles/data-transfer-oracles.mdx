...
As part of the migration to Solana, the data transfer architecture is moved off-chain to Oracles,
which increases scalability and reduces points of failure. In the new architecture, the blockchain
components are decoupled from the data transfer components, which means that blockchain outages or
congestions will no longer disrupt IoT data transfer.

## OpenLNS

Before the architecture upgrade, Helium Console was the only choice of LoRaWAN Network Server (LNS)
on the Helium Network as the old architecture requires the LNS to administer both LoRaWAN specific
functions as well as blockchain overhead. Now that blockchain components are no longer intertwined
with data transfer, IoT application can now leverage any compatible LoRaWAN Network Server. Check
