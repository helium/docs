...
There are initially three Rewards Oracles, one for each subnetwork (Mobile and IoT) and one for HNT,
...
Receipts, and the veHNT backing the Mobile Network and emits rewards to each 5G Hotspot.
...
The following comprise the subnetwork's DAO Utility Score:

- subnetwork's DCs burned in USD
- subnetworks's active device count and device activation fee
- veHNT backing each subnetwork

DC burned, active device count, and locked veHNT are submitted to the chain through the proxy of
...
- To learn more about how locking tokens and governance will change, refer to the documentation on
