...
Post the transition to Solana, all participants within the network who receive tokens are classified as "Rewardable Entities."

Initially, only Hotspots had the capability to earn rewards. This scope broadened to encompass mobile phones utilizing the Helium Mobile app for Mobile network mapping. Looking ahead, additional avenues to earn rewards will be introduced, such as utilizing specific mapping devices.

Within the Helium Network, a rewardable entity is symbolized on Solana as a Non-Fungible Token (NFT).

NFTs are well-suited for this purpose as they denote ownership in a manner that is readily transferable and universally recognized by major wallets.
