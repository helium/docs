...
Hotspots were the first rewardable entity, this changed with the addition of phones with the Helium
Mobile app installed mapping the Mobile network. Dedicated Hardware mappers and other rewardable 
categories will be available in the future.
...
cryptographic proof to the blockchain that verifies the NFT and ownership. In other words, instead
of sending a transaction with:
