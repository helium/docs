...
2. The wallet to be used requires enough HNT to burn for joining the virtual device to the network.
   ...
   - See [Setup Data Only Hotspot](/hotspot-makers/iot/data-only-hotspots/#transactions--cost)
...
[instructions](/hotspot-makers/iot/data-only-hotspots/) for creating and **funding this wallet**.
...
Log on to Console, and [create a new device](/use-the-network/console/adding-devices/).
...
[configuration profiles](/use-the-network/console/profiles/) in Console for this device, such as
`rx_delay`.
