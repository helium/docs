...
2. The wallet to be used requires enough HNT to burn for joining the virtual device to the network.
   ...
   - See [Setup Data Only Hotspot](/network-iot/hotspots-iot/data-only-hotspots/#transactions--cost)
...
[instructions](/network-iot/hotspots-iot/data-only-hotspots/) for creating and **funding this
wallet**.
...
Log on to Console, and [create a new device](/network-iot/console/adding-devices/).
...
Consider making changes to settings within [configuration profiles](/network-iot/console/profiles/)
