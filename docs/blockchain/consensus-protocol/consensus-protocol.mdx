...
* **Permissionless** - Any Miner operating in accordance with the consensus rules and network specifications should be able to participate freely in the Helium Network. 
...
A new Consensus Group (CG) is elected once per epoch based a combination of factors. Currently there are `16` Miners elected to each consensus group, as defined in the `num_consensus_members` chain variable.

All Miners active on the Helium Network are eligible to be elected to a consensus group. The two primary factors used as inputs to any given election are:
...
* Once elected, a Miner can typically be part of up to four consecutive Consensus Groups.  
* Poorly performing CG members are likely to be deselected before their four epoch limit is reached. 
* Once a Miner is deselected from a Consensus Group, it's not eligible to be elected to another group until the other 15 members that were in its group are completely cycled out. This would be four elections into the future.


## The Mining Process

Following the election of a new Consensus Group, a distributed key generation phase occurs to bootstrap the threshold encryption key (TPKE). This TPKE is a cryptographic primitive that allows any Miner in the network to encrypt transactions to a master public key (PK) such that the Consensus Group must work together to decrypt it.

Hotspots are constantly submitting new transactions to the current Consensus Group. As they arrive, each CG member takes a random subset and forwards them around to other members of the group after encrypting them with their share of the master public key.

## Rewarding Hotspots

At the end of each epoch, mining rewards are distributed by the CG to the wallet addresses that have earned them. Currently `66%` of all mining rewards go to the hotspot infrastructure (with the remaining `34%` being distributed to Helium, Inc and other network investors). A graphical overview of token reward system can be [viewed here](https://www.helium.com/hnt) and full description of mining per epoch [is here](../blockchain/mining).

During the course of any epoch, Mining are rewarded for the following list of activities:
