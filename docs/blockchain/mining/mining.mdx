...
- `(43790.40 minutes per month / 30 minutes per epoch) = 1459.68 epochs per month`
- `(2500000 tokens minted per month / 1459.68 epochs per month) = 1712.704154343 HNT per epoch`
...
As calculated above, the target `HNT` per epoch is approximately `1712.704154343`. The
...
| :-------------------- | :------------ | :------------------------ |
| PoC Challenger        | .90%          | 15.41433739               |
| PoC Challengees       | 5.02%         | 85.97774855               |
| Witnesses             | 20.08%        | 343.9109942               |
| Consensus Group       | 6%            | 102.7622493               |
| Security Tokens       | 33%           | 565.1923709               |
| Network Data Transfer | _Up to 35%_   | _Up to 599.446454_        |
| **Total**             | **100%**      | **1712.70415434**         |
...
is capped at 35% (599.446454 HNT). Therefore, if the DC burned doesn't equal
the value of `599.446454` HNT, the remaining HNT is redistributed to the Proof of
...
- The remaining `589.446454` from the Network Data Transfer reward would be
  distributed ratably among the Challengers, Witnesses and Challengees.
  - `20.40391572` to the Challenger group
  - `455.2340306` to the Witness group
  - `113.8085077` to the Challengee group
...
- Because the Network DC burn exceeded the `599.446454` available to the
...
- Hotspots are only eligible to submit one Proof of Coverage Challenge - which
  results in them earning the `PoC Challenger` reward - **once per 480 blocks**.
...
- As with any epoch, we would distribute the target of (approximately) `1712.70415434`
  HNT.
