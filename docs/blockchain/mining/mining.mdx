...
pagination_label: Mining & Rewards
...
| PoC Challenger        | poc_challengers_percent | Rewarded to any **Validator\*** that creates a valid PoC challenge and submits the corresponding receipt to the blockchain. (see note below) |
| Witnesses             | poc_witnesses_percent   | Distributed to selected Hotspots that witness a beacon packet as part of a PoC Challenge.                                                    |
...
[HIP55](https://github.com/helium/HIP/blob/main/0055-validator-challenges.md) moved the
resposibiility of creating PoC Challenges to Validators, and as such PoC Rewards (chain_var
`pos_challengers_percent`) are now awarded to any Validator that creates a valid PoC challenge and
submits the corresponding receipt to the blockchain.
...
| PoC Challengees       | 4.73% - 12.23%  | 82.1247 - 212.331         |
| Witnesses             | 18.92% - 48.92% | 328.4989 - 849.3323       |
...
- The remaining `641.0417` from the Network Data Transfer reward would be distributed by weight
  among the Witnesses and Challengees.
  - `128.2083` to the Challengee group => `4.73/(4.73+18.92) = 20%`
...
- Validators use the `poc_challenge_rate` chain_var that defines the target number of Challenges per
  block. See
  [HIP55](https://github.com/helium/HIP/blob/main/0055-validator-challenges.md#poc-challenge-creation-and-the-consensus-group)
  for more details.
- `Challenger` rewards are distributed to Validators that create and collect Challenge data and
  submit this information to the blockchain.
...
impacted equally** (except for Consensus Group members; more on this below). So, although it's
annoying that the effective rate of HNT may have dropped over a given period of time, know that
