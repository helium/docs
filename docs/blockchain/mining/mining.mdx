...
As of August 1, 2021 (the most-recent halving), the target production rate for new $HNT minted per month is `2,500,000,000`. This
means that, if the blockchain performs as designed, it will produce 
`2,500,000,000` HNT per month. This target rate is based on the following two 
parameters, as defined in their specific chain variables:
...
epoch of `30` blocks, the blockchain will produce `2,500,000,000` HNT per 
month. Per epoch, this equals roughly `1,712,704.154343` HNT. The math for this 
is as follows:

- `(43200 minutes per month / 30 minutes per epoch) = 1440 epochs per month`
- `(2,500,000,000 tokens minted per month / 1440 epochs per month) = 
1,736,111.1 HNT per epoch`
...
As calculated above, the target `HNT` per epoch is approximately `1,736,111.1`. 
The next logical question is, "Where does all this HNT go?" Let's take a look.
...
| :-------------------- | :------------ | :------------------------ |
| PoC Challenger        | 0.90%         | 15,625.0               |
| PoC Challengees       | 5.02%         | 87,152.7               |
| Witnesses             | 20.08%        | 348,611.1              |
| Consensus Group       | 6%            | 104,166.6              |
| Security Tokens       | 33%           | 572,916.6              |
| Network Data Transfer | _Up to 35%_   | _Up to 607,638.9_      |
| **Total**             | **100%**      | **1,736,111.1**         |
...
is capped at 35% (607638.9 HNT). Therefore, if the DC burned doesn't equal
the value of `607,638.9` HNT, the remaining HNT is redistributed to the Proof of
...
- In this scenario, total HNT value of DC transferred in this epoch is `400HNT`.
  This calculation is: `(2,000,000 DC * $0.00001 / $0.05 HNT Oracle Price)`
- These `400 HNT` would be split proportionally to the Hotspots who did the work
  routing packets at the 1:1 rate.
- The remaining `607,238.9` from the Network Data Transfer reward would be
  distributed ratably among the Challengers, Witnesses and Challengees.
  - `20,977.34` to the Challenger group
  - `469,009.19` to the Witness group
  - `117,252.27` to the Challengee group

**Example 2: DC Burn exceeds 35% HNT**

- In a given epoch, `5,000,000,000` DCs are spent on data transfer across the
  network
- The HNT Oracle Price is `$0.05`

In this scenario, total HNT value of DC transferred in this epoch is 
`1,000,000HNT`. This calculation is: `(5,000,000,000 DC * $0.00001 / $0.05 HNT 
Oracle Price)`

- Because the Network DC burn exceeded the `607,638.9` available to the
  Network Data Transfer reward, all Hotspots who did the work over this epoch 
  with split the full `1,163,194.5HNT` proportionally.
...
## HIP39 and the Redenomination of HNT
On November 20, 2021 the community voted to redenominate the Helium Token 
through 
[HIP39](https://github.com/helium/HIP/blob/master/0039-hnt-redenomination.md).
**HIP39 was activated Febuary X, 2022... todo:link & actual date* 
Prior to the redenomination, one HNT was equivalent to 100,000,000 bones -
the atomic units of the Helium Token. (Like Satoshis for BTC). With 
redenomination, one HNT is equivalent to 100,000 bones.

**What Did This Mean for the Helium Token?**

If you currently have 1,000 HNT worth $0.04 each, that would've been 
denominated as 1 HNT worth $40 before the redenomination.

`1000 HNT * $0.04 = $40` = `1 HNT * $40.00 = $40.00`

Similarly, if you had previously been mining 0.25 HNT per day, that would now 
be 250 HNT per day. Matching the above scenario, that would be worth $20 in 
both situations.

**A Bit More about Bones**
...
uses something called `proration` to ensure that the target of `2,500,000,000` is
...
then `2,500,000,000` new HNT will be produced. "One month" is a period of time
...
times are slower to ensure the `2,500,000,000` per month target is hit?_ No. Again,
