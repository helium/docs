...
<img
  ...
  src={useBaseUrl("img/blockchain/packetpurchasing.png")}
/>
...
transaction channel, a transaction which also shows how many data credits are
being burned in the name of which hotspots. Should a hotspot not be credited the
appropriate amount, it has recourse during a "grace period" the ten blocks after
the closure to file a dispute, using the signed offers as evidence. This is done
in the form of alternate state channel close transactions.
