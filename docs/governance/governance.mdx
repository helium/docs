...
technical architecture, the tokens' economic model, or meta-governance changes. It is governed by
the global community of Helium token owners. These token owners include Hotspot deployers, network
users, wireless network operators, hardware manufacturers, and more. Tokens must be locked for
voting on network changes, ensuring long-term stakeholder alignment.
...
community calls. Any Helium token owner is eligible to vote on HIPs. To vote, a Helium token owner
must first lock their tokens for a period of time, up to four years, which grants the owner voting
power determined by the number of locked tokens and the lock duration.
