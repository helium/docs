...
- **Node** - Refers to individual devices or computers that participate in maintaining the network
  ...
  consensus.
- **Oracle** - Oracles are "off-chain services that serve as bridges between the external world and 
  the blockchain. The Helium Oracles handle all data about the network, except for transactions handled 
  by the Helium Program Libraries.
- **Validators** - A distributed network actor responsible for verifying and validating transactions
  and blocks, maintaining the network's security and consensus. Helium on Solana does not currently 
  use validators.
...
2. **GitHub Repository**: The [HIP GitHub](https://github.com/helium/HIP) repository provides access to the Helium Improvement
...
4. **Monthly Community Calls**: The Helium Community holds monthly video calls (led by the Helium
   ...
   an [open agenda](https://www.helium.foundation/community-call), and any community member can join and add items to the agenda to be discussed.
...
[realms-tutorial]: /staking-with-realms
