...
The Helium Network serves as the overarching system that enables both the Mobile and IOT networks to
exist via the HNT token.

Those who delegate their veHNT position to either network and contribute regularly to governance are
eligible to receive a portion of HNT rewards. The distribution of HNT tokens available to either the
Mobile or IOT network is determined by the amount of veHNT delegated to either.

### The Mobile network

The Mobile network is dedicated to the growth and expansion of the Helium Network in cellular
communications. Delegating veHNT to the Mobile network will help finance projects and initiatives
aimed at integrating Helium technology into cellular devices and networks.

### The IOT network

The IOT network focuses on supporting the growth and development of IOT devices and applications.
Delegating veHNT to the IOT network will help fund projects and initiatives that further the IOT
Ecosystem on the Helium Network.

## Delegating veHNT

To earn HNT reward, veHNT holders must delegate their position to either the Mobile or IOT networks
(or both). The lock-up period can range from 1 day to 48 months, with the longer lock-up periods
...
veTokens can be managed through [Realms](/governance/staking-with-helium-vote) using the in-app
browser on the [Helium Wallet App](/wallets/helium-wallet-app):
