...
HIP 51- 53, the Helium Network established the distinction between Networks and subnetworks, then
represented by a token for each. HIP 51 - 53 also established the Vote Escrowed Model of token
Staking for Voting Power.

More recently, HIP 138 returned governance to single token governance using just HNT, and removed
direct governance by the IOT and MOBILE tokens.
