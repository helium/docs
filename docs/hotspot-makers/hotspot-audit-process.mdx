...
The Hardware Audir will require two hardware samples of each Hotspot. For example, if the Audit
...
The [Manufacturing Compliance Committee](/hotspot-makers/compliance-committee) is only currnetly
accepting Light Hotspots and 5G Hotspots for audit at this time.
...
2. Provide the architecture/distribution used by the CPU.
   - Match it here:
     ...
     configuration.
3. Provide the commands to start and stop the `gateway-rs` executable (ie, `helium_gateway`)
4. If using the ECC608 as a security element, provide the key slot and bus address
5. Upon power up, is `gateway-rs` running and ready to audit? **Y/N**
   - `gateway-rs` must automatically update; the Auditor shall not update the firmware.
6. Is the Hotspot using Helium's `gateway-rs` updater? **Y/N**
...
1. Provide instructions for stopping and starting Miner.
2. Provide location of miner data.
...
3. Hardware items to illustrate include (but aren’t limited to):
