...
* **Blockchain** - This is a ledger of all transactions that have taken on the network. It is stored and run on both hotspots and validators.
* **Validators** - These are servers that run the blockchain to offload computation and storage from the hotspots. In the future, hotspots will not have a copy of the blockchain but will communicate to the validators to perform blockchain transactions.
* **Helium Router/LoRaWAN Network Server** - Router is equivalent to a LoRaWAN Network Server (LNS). LNS manages low level network communications with devices through hotspots. By running your own LNS, you have complete control over additional LoRaWAN network commands that allow you to customise how your devices communicate to the network. Only for those who really know their way around LoRaWAN!
...
<img src={useBaseUrl("img/large-device-fleet/helium_architecture_simple.png")} title="Helium Network Architecture"/>


## LoRaWAN and Blockchain in Helium
<img src={useBaseUrl("img/large-device-fleet/helium_architecture_simple_2.png")} />
