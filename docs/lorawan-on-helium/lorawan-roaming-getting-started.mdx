...
slug: /lorawan-on-helium/lorawan-roaming-getting-started
---
...
7. Next, let’s initialize some environment variables so that you don’t have to tell the CLI the same
   ...
   1. Run `./main env init`
   1. Fill in `http://maininet-config/helium.io/6080` for `Config Service Host`
...
12. Lastly, define other relevant parameters for HTTP roaming. You can run
    ```bash
    ./main route update hhtp --dedupe-timeout <dedupe-timeout> --path <url path> --auth-header <auth-header> --route-id <route-id> --commit
    ```
