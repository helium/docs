...
To submit Hotspots for Denylist consideration, start by clicking on the "Submit Denylist Request"
button on the Crowdspot.io landing page.

Once in the Submission form, users will be given the option to select the type of submission:

- Submit additions to the Denylist
- Request removals from the Denylist
...
and inserting more Hotspot `b58` addresses.
...
When reporting a Cluster of Hotspots, include all `b58` addresses **and** toggle on the "Is this a
cluster of Hotspots" checkbox. This information assists in evaluating the legitimacy of the Hotspots
in the request.
...
Still, when this _does_ happen, Hotspot Owners can potentially submit information and evidence to
...
There is a limit of 3 URLs per submission.
...
- For a single image, we recommend using a free image-to-link converter and paste the URL here.
...
The Trust Score Algorithm compares these metrics to a sample of known spoofers from the Denylist and
a sample of known legitimate Hotspots.

Crowdspot will leverage algorithms to increase efficiency and accurately detect gaming Hotspots.
Trust Scores will be assigned to every submitted Hotspot to assist in the evaluation. A Trust Score
is a score assigned to Hotspot based on a combination of metrics found on Crowdspot.io. These
metrics are primarily graph data and include the Hotspot Witness graph, the Witness SNR vs. RSSI
graph, and the Witness RSSI vs. Distance graph. The Trust Score compares these graph metrics to a
sample of known gaming Hotspots from the Denylist, as well as a model of known legitimate Hotspots,
to determine the relative legitimacy of the Hotspot.
