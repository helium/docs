...
1. Use the Raspberry Pi Configuration application and enable the SPI kernel
   module in the Interface Options section. This is located in the GUI from
   the Applications Menu under Preferences
...
There are two files that the LoRa Packet Forwarder uses for it's configuration:
- global_conf.json
- local_conf.json

Make sure you are editing/working with the correct file.
...
1. Change directory to the /etc/lora-gateway directory
   ...
   cd /etc/lora-gateway
   ```
3. Copy the US915 Band 2 configuration from the cfg subdirectory as
   global_conf.json
   ```
...
3. Generate a unique gateway ID based on the MAC address of one of your network 
   ...
   ```
   This command will generate you one or more unique values you can use for 
   the gateway EUI. 
...
In the next step we will be using the `DCA6328XXX70` value as an example.
Do not use the example value from this documentation, it will not work.

### Point the LoRa Packet Forwarder to the Helium Gateway

By default the LoRa Packet Forwarder from Dragino is configured to work with
the Things Network.   Here we will configure it to talk to the Helium
gateway you installed on the Raspberry Pi.
...
2. Replace the current value for gateway_ID with (one of) the values generated
   in the previous step and **append the value FFFF to the end**.  The 
   reason we append `FFFF` to the end is the gateway ID is required to be
   16 characters, and the unique MAC address generated is only 12.  Again,
   this guarantees your gateway ID will be unique in the network.
...
/etc/lora-gateway/local_conf.json file. Be sure to double
check the JSON syntax and have the key/values separated by commas and the {}
characters match up.
...
communicating on.  **Don't** use the value assigned by your router or your 
public IP address.
...
1. To view your gateway address and unique animal name assigned to you
   ...
   ```
2. Backup your gateway key to another machine/media for recovery purposes. 
   The file `/etc/helium_gateway/gateway_key.bin` contains the key.
