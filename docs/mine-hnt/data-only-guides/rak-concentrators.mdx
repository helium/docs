...
:::important To use the concentrator module with a Raspberry Pi HAT, you must
make sure to order the variation with the SPI interface. :::
...
:::note `ttn-gateway` is just the name for the LoRa packet forwarder service in
the configuration for these RAK concentrators; this is like a "legacy name" that
...
:::important Every time a new region is selected using the _Setup RAK Gateway
Channel Plan_ option, these settings are replaced with the default
configuration. So, make sure to keep this in mind if you modify the region
later. :::important
