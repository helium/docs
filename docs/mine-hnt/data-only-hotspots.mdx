...
:::warning
Adding Light Hotspots is not yet enabled on chain, but is expected to be
released in early July. This guide is released for preparation only.
:::
...
with location asserted, they will earn data credits when forwarding network
data.
...
user's wallet
* **assert location**: provides GPS location, altitude, and dBi antenna
output for the hotspot

The fee schedule is as follows:
* **add data only hotspot**: 500000 DC ($5)
* **assert location**: 1000000 DC ($10)
...
After you verify the transaction, use append the command with `--commit`
...
You can track the transactions progress in being processed by using the
...
light hotspot's keypair. It's location can be customized in the client's
`settings.toml`, but by default, it's located at
`/etc/helium_gateway/key.bin`
:::

## Assert Hotspot

To assert the gateway using the `helium-wallet hotspots assert`
```
helium-wallet hotspots assert --gateway LIGHT_GATEWAY_PUBKEY --lat=37.7806295 --lon=-122.4008481
```
...
Once the transaction clears, you're data only hotspot is ready to start
earning!
