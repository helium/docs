...
released in early July. Earning $HNT for data transfer is expected to be enabled at a later time as part of Milestone 2. Please look for updates in #gateway-development channel. This guide is released for preparation only.
:::
...
To assert the Hotspot using the `helium-wallet hotspots assert`
```
