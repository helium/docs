...
slug: /mine-hnt/full-hotspots/become-a-maker/security-requirements
---
...
and libp2p transactions. The three word animal name (e.g. angry-purple-tiger) is
actually derived from the public key of the `swarm_key`.

:::

While formalizing the audit process, the Helium Foundation has determined that
the only approved approach to guaranteeing this at this time is using the ECC608
to generate and store the key during manufacturing. Other approaches are
possible, but they would require case-by-case consideration and the potentially
a third-party audit.
