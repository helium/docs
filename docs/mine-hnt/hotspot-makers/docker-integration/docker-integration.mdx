...
maintains firmware images for the original Helium hotspots (Raspberry Pi 3B+ and
4 based). When we have a release candidate, you will see a tag with the date of
the release candidate. This usually means that we are testing a release
candidate. This testing process has many stages, but if and when a release
passes, it will graduate to "General Availability" and be re-tagged with the
`_GA` suffix. This triggers an automatic build of the Docker images which get
pushed to Quay when complete. In addition, a "Blockchain Release" will be posted
on [engineering.helium.com](https://engineering.helium.com/).
...
Other than in the event of an emergency, we generally give actors about a week
to update their images. However, it is generally recommended to automate the
process by checking our Quay every half hour or so.
...
Start by following
[these instructions](https://docs.helium.com/mine-hnt/build-a-packet-forwarder#run-the-miner).
...
Setting `REGION_OVERRIDE` is only useful if you want a hotspot to forward
...
In fact, in Helium's own firwmare image, we actually use this information to
also determine which packet forwarder configuration to select, so our startup
routine for packet forwarder will actually block as it requests region from
miner
...
When the user has asserted the Miner and its location to the blockchain, you can
get the appropriate region from Miner and use the appropriate configuration for
a packet forwarder.
...
Docker container, and connect D-bus to the Docker container:
