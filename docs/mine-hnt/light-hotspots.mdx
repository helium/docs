...
- Change how Hotspots route data packets by replacing its `state_channel` client from erlang to rust, taking advantange of the improvements in gRPC and data streaming (complete)
- Update how Hotspots receive, emit, and acknowledge proof-of-coverage receipts from erlang to rust (dev complete, testing on testnet, targets milestone 4)
- Test PoC in testnet (started, targets milestone 4)
- Update how full Hotspots communicate from libp2p to gRPC and follow a Validator for ledger information instead of syncing a local copy of the blockchain (dev complete, testing on testnet, targets milestone 4)
