...
Debian package is also the simplest to deploy and maintain when running multiple validator instances on a single host.
...
- VALIDATION_WIDTH - Maximum number of concurrent transaction validations that the miner will do when
