...
You should be comfortable with running a Testnet Validator, have experience with Unix and Docker, and understand basic system administration (security, port mappings) to to run a Validator. There is no undo button in the blockchain!
...
- [Build the Validator from Source](#deploy-miner-from-source)
...
- `--name validator` names the container, which makes interacting with it easier, but feel free to name it whatever you want.
...
Okay, brave soul. Here's what you'll need to build and deploy the miner from source:
...
- `nat_type` of `none` is best for validators. Anything else (`unknown` `symmetric`, `restricted`) means the validator code thinks it is behind some kind of NAT.
- `height` is the currently synced block. Compare to the [Explorer block height](https://explorer.helium.com/blocks).
