...
- At the end of the election, all Validators vote and sign the election result. If the election fails, Validators that a majority agree did not sign the election results are penalized. Failed elections are significant to the network as they prevent new groups from being elected and rewards to be issued to all network participants including Hotspots.
...
- The amount applied is a chain variable, which was originally set at `1.0` and was changed to `5.0` as of block 1,107,995. The variable is viewable by running `miner ledger variables dkg_penalty`.
