...
- Overall Baselining (do this after the validator is caught up to the tip of the chain, and running normally for a few hours)
  ...
  - You can review your Disk Bandwidth using `sar -d -p | egrep "sd[a-z]|DEV"` NOTE: this command line is setup for using `/dev/sd<some letter>` device names if your hardware is different you may need to change this commnd as needed.
