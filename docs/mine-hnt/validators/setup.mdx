...
--name validator \
--mount type=bind,source=/home/ec2-user/validator_data,target=/var/data \
quay.io/team-helium/validator:miner-val-{arch}_val{version}
```

Replace`{arch}` with the architecture used, i.e. `amd64` or `arm64`, and `{version}` with the version number of the image. We advise not using the latest tags as the underlying container will not be consistent.
