...
Before running the container for the first time, it is advisable to pick a "system mount point", i.e., a directory in your host system; some long-term data is stored there. This allows you to easily maintain your validator's blockchain identity (i.e. swarm keys) and blockchain state through image updates. The validator directory should be created on a persistent ebs volume (if aws) or something similar that lives across server lifetimes.
...
--name validator \
--mount type=bind,source=/home/ubuntu/validator_data,target=/var/data \
quay.io/team-helium/validator:miner-val-ARCHVSN
```

Replace`ARCHVSN` with the architecture used, i.e. `amd64` or `arm64`, and with the version number of the image. We advise not using the latest tags as the underlying container will not be consistent.
...
The ```--mount``` with the parameters above will mount the container's ```/var/data/``` directory to the systems directory ```/home/ec2-user/validator_data```. You will want this folder to be persistent across runs of the docker container as it will contain both the blockchain data and the miner key of your validator. 
