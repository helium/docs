...
The ```--mount``` with the parameters above will mount the container's ```/var/data/``` directory to the systems directory ```/home/ubuntu/validator_data```. You will want this folder to be persistent across runs of the docker container as it will contain both the blockchain data and the miner key of your validator. 
