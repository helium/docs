...
This guide will cover the end-to-end process of deploying a Helium Validator on Testnet. It assumes you're starting from scratch. At the end, you'll have a Helium Validator deployed on the Network.
...
And checkout the latest testnet val tag. Consult GitHub for the current version and replace it into this command:
...
- LOG_ROOT (default: `log`) - Directory location of log files. May be absolute
  or relative path. Relative paths are relative to miner install location.
- UPDATE_DIR (default: `/opt/miner/update`) - Location tha the miner looks in
...
- GRPC_PORT (default: `8080`) - Port used by light gateways to interact with the
  validator. This port must be public accessible.
