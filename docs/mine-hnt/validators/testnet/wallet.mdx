...
For testnet, the cooldown period is shortened to 180 blocks, approximately 3 hours. In the above calculation, replace `250000` with `180`.
...
Once the unstake process is completed, the validator process may be terminated and destroyed. A validator that has been unstaked will not earn rewards and cannot be restaked after it has been unstaked.
