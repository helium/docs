...
If you aren't yet familiar with the penalty system, check out the [description here](https://docs.helium.com/mine-hnt/validators/penalties/).

Low-grade Performance and DKG penalties are very noisy and difficult to troubleshoot.

Based on early data, it appears that if your validators have penalties where the sum (Performance + DKG) is less than (Tenure), your validator is performant in-line with expectations.

The most common cause of Perf/DKG penalties is bad luck (due to bad peers or otherwise).

A few additional sources of possible problems exist, each requiring detailed metrics monitoring capabilities to troubleshoot.
