...
* On a running Validator run `miner snapshot list`
* Find the highest "Height" block and download the snapshot from Helium via `curl -O https://snapshots.helium.wtf/mainnet/snap-<height>`
...
* Load snapshot from downloaded file ```miner snapshot load /var/data/snap-<height>```
...
NOTE: If you build the Validator from source the paths may be different
...
For the time/blocks when your validator accrued penalties, check your metrics dashboard (e.g., Grafana) [Baselining Your Validator](https://docs.helium.com/mine-hnt/validators/requirements).
