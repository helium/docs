...
- `--interval 7200` Poll interval (in seconds). This value controls how frequently watchtower will poll for new images. Every 2 hours for the purpose of testnet is recommended.

Additional flags that may be helpful:

- `quay.io/team-helium/validator:latest-val-amd64` Include the image text as the last line in the command above. Including this at the end simply tells watchtower to only monitor the validator image and not an of the other images in your docker server. This is included here for convenience if there are other docker containers running on the host. If the validator is the only service running, this can be ommitted.
...
Head over to the [Helium Testnet Explorer's Validators Page](https://explorer.helium.wtf/validators) and search for your validator. This may take 10 to 15 minutes from the time that your Validator is online and staked. You should see your validators name (short-umber-bull in the example above) listed. It may be easier to sort by #, Decending, as your new Validator will be near the top of the list.
