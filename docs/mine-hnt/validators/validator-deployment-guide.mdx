...
Start by cloning the validator testnet branch of `miner`:

```
git clone -b adt/erlang-tc https://github.com/helium/miner.git
```

Then proceed to get Erlang (here we're specifying `22.3.1`):
...
Acquire the needed dependencies:
...
Then we need to unpack and install. Start with `libwxgtk`:
...
Force the install of required dependencies from the Erlang install by running the following:
