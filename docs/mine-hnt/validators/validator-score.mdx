...
This penalty of `1.0` is added even if the Validator is ejected from the consensus group as it is a penalty added for participating in the ending consensus group.
...
* This penalty is applied for each occurrence, which can happen multiple times during an consensus group.
...
* This penalty is applied once per consensus.
