...
The Helium Foundation has implemented HIP-70 and migrated our L1 to Solana. Voting on Helium 
Improvement Proposals is performed with vote eskrowed tokens.
...
Now the Solana migration has completed, stakers can use their staked veHNT to vote on proposals 
...
Staking HNT for veHNT will be done on [Realms](https://realms.heliumvote.com/dao/hnt)
See the [Staking veHNT on Realms](https://docs.helium.com/vote-escrow/realms) documentation
...
The veHNT decay countdown starts when initiated from the Wallet app/Realms, either immediately
in "cliff" mode or after a "constant lockup" is unlocked and cooldown started.
...
Yes. You can increase your lock-up period by extending to the maximum of 48 months at any time, and
the increase activates at the start of the next epoch. If the veHNT is delegated you must undelegate, 
change the period and redelegate and lose delegation rewards within that 24 hour Epoch. If those three 
actions take longer than an epoch or are split over two epochs then delegation rewards are lost for two
Epochs.
...
Yes a constant rolling lock-up period is available called Constant Lock-up. The decay
