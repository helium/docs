...
Every epoch, a fixed amount of HNT is allocated to all subDAOs. Each subDAO receives a share of this fixed amount. The sizes of the shares are determined by the Utility Scores of each subDAO of Helium.

The higher the Utility Score of a subDAO is in relation to the Utility Scores of all other subDAOs combined, the higher its share of the total HNT allocated to all subDAOs. As the DAO Utility Scores are calculated each epoch anew, the share of HNT a subDAO receives may change from epoch to epoch. 
...
1. Increasing the amount of data usage on the subDAO, i.e., the Data Credits burned. This increases the `D`
   value in the Utility Score.
2. Increasing the Data Credits used to onboard the number of Hotspots for that subDAO. This increases the `A`
   value in the Utility Score.
