...
description:
  How to access and decode Helium Oracle reward and data files stored in AWS S3, with protobuf
  schemas and code examples.
image: https://docs.helium.com/img/link-image.png
slug: /network-data/oracle-data
---

The Helium Network operates Oracles that perform various tasks backing data transfer and rewards
across the Helium subnetworks. These Oracles provide extensive logs for analysis of the Helium
subnetworks.
...
under the top-level data category prefixes listed below. All data in the S3 bucket uses 'Requester
...
For more information on accessing data from an AWS requester pays bucket, refer to the
...
Under each top-level data category prefix, the related data files are provided in the prefix
...
access patterns are used will greatly reduce the cost burden for accessing the data.

Regarding access patterns, one approach to minimize the quantity of S3 API calls over time, and save
money, is to use the `aws s3api list-objects-v2` command and specify the `--start-after` and
...
set to the `top-level data category` + `file prefix` of the desired file, as indicated below:
...
The raw data stored in the `foundation-poc-data-requester-pays` bucket is in encoded protobuf format
in `.gz` compressed files. The table below provides the top-level data category prefixes, the file
types contained within each data category prefix, the file prefixes beginning each filename, the
filename patterns for each of the file types, and links to the corresponding protobuf used to decode
the data.

:::warning Proof-of-Coverage data is deprecated

Proof of Coverage has been removed from the Helium networks as of July 6, 2026. The
Proof-of-Coverage data types listed below, such as IoT beacon, witness, and entropy reports, are
deprecated and kept here for historical reference. Existing files remain accessible in the bucket.
...
| `foundation-mobile-verified`        | `SpeedtestAvg`                           | `speedtest_avg`                               | `speedtest_avg.<unix_ms_timestamp>.gz`                               | [`service/poc_mobile.proto`<br />(Line 226: `speedtest_avg`)](https://github.com/helium/proto/blob/953da03fd03f1f655e7c27328f068fa9654d1954/src/service/poc_mobile.proto#L226)                                        |
| `foundation-mobile-verified`        | `VerifiedSpeedtest`                      | `verified_speedtest`                          | `verified_speedtest.<unix_ms_timestamp>.gz`                          | [`service/poc_mobile.proto`<br />(Line 36: `verified_speedtest`)](https://github.com/helium/proto/blob/6cfd6dc952349e31187c7071dbbcd86780eb97e6/src/service/poc_mobile.proto#L36)                                     |
...
```protobuf
// poc_mobile.proto
// copied from https://github.com/helium/proto/blob/953da03fd03f1f655e7c27328f068fa9654d1954/src/service/poc_mobile.proto#L43
...
protobuf.load('poc_mobile.proto', function (err, root) {
  if (err) throw err
