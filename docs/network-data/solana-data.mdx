...
description:
  Developer guide to reading and writing Helium data on Solana, covering accounts, PDAs, tokens,
  Anchor SDKs, and program IDs.
...
<img className="docsheader" src={useBaseUrl('img/solana/solanaheader.png')} />
...
<!-- prettier-ignore -->
```js
PublicKey.findProgramAddressSync([Buffer.from("helium", "utf8")], helium_sub_daos.programId)
```
...
Whereas the owning wallet of the `TokenAccount` is the `TokenAccount.owner`. You can see a
[token account for a testing token we created](https://explorer.solana.com/address/BM9kx8NZGNThVr6HnBJo1CR5Jvk5Zc3EEy7wWztaJHS2?cluster=devnet).
...
The classic way of fetching data on Solana is to use the @solana/web3.js connection object.
...
console.log(tokenAccount.amount, tokenAccount.owner)
```

In the case of Helium programs, they all use a framework called
[Anchor](https://github.com/coral-xyz/anchor), which amongst other things generates SDKs for us.
...
That means the interface to all instructions is predictable. You should be able to look at either
...
others are paid. The Foundation uses Helius as their RPC Provider, and it's useful to use an RPC
provider that is familiar with compression. They will all support this at some point.

You can read more about [RPC & RPC Nodes](https://solana.com/rpc).
...
- Allows off-chain Oracles to distribute rewards to rewardable assets (such as Hotspots) that are
  represented as NFTs.
