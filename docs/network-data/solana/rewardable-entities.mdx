...
description:
  How Hotspots are represented as Solana NFTs, including fetching by wallet, ECC key lookup,
  onboarding, and claiming rewards.
...
Hotspots on the Helium Network are currently uniquely identified by their ECC Compact Public Keys.
...
onboard Mobile Hotspots. Each subnetwork must approve Makers to issue rewardable entities.
...
In practice, we have made it so that this can all happen in a single transaction for reward claims.
...
If you are interested in using the REST API, you can see the relatively simple
[client code](https://github.com/helium/helium-program-library/blob/master/packages/distributor-oracle/src/client.ts).
