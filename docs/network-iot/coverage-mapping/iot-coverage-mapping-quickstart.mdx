...
title: IoT Mappers Quickstart
pagination_label: IoT Mappers Quickstart
sidebar_label: IoT Mappers Quickstart
...
development devices, [CayenneLPP](https://docs.mydevices.com/docs/lorawan/cayenne-lpp) is often
used. For this guide we'll be using a decoder specifically made for this tracking device. We have a
repository with decoders for various devices on
[GitHub](https://github.com/helium/console-decoders).
...
2. Copy and paste the complete decoder function provided by the manufacturer or found
   ...
   the API specification in [Mappers API](/iot/coverage-mapping/api). The required fields are
   case-sensitive. If you got your decoder from the manufacturer and one or more required fields are
   missing - add them with hard-coded values like accuracy in the above example.
