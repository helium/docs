...
cable. Again this may vary depending upon your target board.
...
If you have not have not already used PlatformIO to build, upload and verify the operation of a
sample application it is best familiarize yourself with the basic use of PlatformIO before adding
...
Before proceeding with actual code that will connect to the Helium network, create a sample,
template project.
...
- For location, if the checkbox is selected the project will be created within the default
...
wizard targeting our CubeCell board.
...
code below. A console quick start guide can be found [here](/network-iot/console/quickstart).

### Sample Helium Network Access Code <a name="ADD-HELIUM-NETWORK-CODE"></a>

If you have not created a simple template project and you want to a try one of the samples you can
...
For our example using the Heltec CubeCell and it's runtime the following piece of code has been
...
Heltec device is configured targeting the North American market. Please follow this
...
require the terminal emulator be disconnected from the target device prior to attempting to upload a
new binary.
...
[here](/network-iot/console/integrations).
