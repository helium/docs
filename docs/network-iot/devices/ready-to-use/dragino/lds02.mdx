...
unused.) Follow our [quick start guide](/archive/console/adding-devices) on how to add a new device
to Console.
...
a decode Function as explained in [our short primer](/archive/console/functions). The manufacturer
of this device, Dragino, is using their own encoding scheme to define which bytes hold which piece
of data. Below is a table taken from the manual showing the size and location of each piece of data.
