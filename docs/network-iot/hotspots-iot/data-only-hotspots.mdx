...
Any LoRaWAN gateway or gateway-capable hardware can be used to pass data through the Helium Network.
Data-only Hotspots are eligible for **Network Data Transfer Rewards** but not **Proof of Coverage
Rewards**. This offers a flexible path for companies and organizations looking to leverage the
...
[IOT Tokens](/tokens/iot-token) when forwarding network data.
