...
the `/usr/bin/` directory. Placing helium_gateway in this bin directory will automatically add it to
the shell path.
...
Helium Wallet CLI. Once onboarded to the network, asserting the location of a data-only Hotspot the
same experience as any other Hotspot.
...
<!--todo: Confirm that this is the right key. If not, where should it be sourced?-->
...
to the network. A utility like [latlong.net](https://www.latlong.net) is helpful to look up
coordinates.

```
helium-wallet hotspots assert --gateway <Hotspot Public Key> --lat=37.771150 --lon=-122.419200 --mode dataonly --commit
```
