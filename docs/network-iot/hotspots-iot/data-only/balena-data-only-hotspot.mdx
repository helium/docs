...
- [RAK 2287 LoRa concentrator SPI based](https://store.rakwireless.com/products/wislink-concentrator-module-sx1302-rak2287-lorawan?variant=41826859385030)
...
Follow the instructions, click `Add a Device` and flash a SD card with that balenaOS image
downloaded from balenaCloud. Enjoy the magic 🌟Over-The-Air🌟!

Find the
[repository for the Data-Only Hotspot with the source code](https://github.com/mpous/helium-data-hotspot)
if you would like to clone and deploy from your computer or you would like to contribute.
...
- Create the variable for all the services `REGION_OVERRIDE` with your LoRa region (e.g. `EU868`).
  You can find a list of LoRa regions compatible on
  [GitHub]](https://github.com/NebraLtd/hm-pktfwd).

### Test the Helium Data-Only Hotspot

Once the Device Variables are properly set up, the Data-Only Hotspot should be running showing some
errors on connecting to the Blockchain on the balenaCloud Logs.

To check that the Data-Only Hotspot has been generated correctly, go to the `Terminal` and select
the `gateway-rs` service, then introduce:
...
To join the Helium IoT Network, at the moment (summer 2021) you only can use the Helium CLI Wallet.
The Helium Wallet App is still not compatible with the Data-Only Hotspots. Follow
...
Now that your Data-Only Hotspot has been added to the Helium IoT Network you should be able to see
it in your Helium Wallet.
...
It's extremely important that at this point, with the Data-Only Hotspot added on the Helium IoT
Network you backup the `gateway_key`. If you lose this key, you will not be able to use this
Data-Only Hotspot anymore in case there is a problem on your Data-Only Hotspot (SD card corrupted,
etc.).

### Backup your gateway_key.bin file

Follow these instructions to back up your `gateway_key.bin` file of your Data-Only Hotspot.
