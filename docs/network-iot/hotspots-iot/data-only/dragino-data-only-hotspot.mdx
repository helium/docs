...
This guide provides instructions on how to configure an 8-channel Dragino LoRaWAN gateway
...
To be able to send and receive LoRaWAN data, your Gateway must be configured to the frequency
...
Your Dragino gateway should now be set up and forwarding LoRaWAN data traffic to the Helium
IoT Network.

### Through Helium Console

If you have a LoRaWAN device configured through the Helium Console, you should now see it
...
Additionally, the LoRa Packet Forwarder log can provide some clues as to whether the LoRaWAN
...
configured correctly. By default, the LoRaWAN Server settings are configured for the TTN EU
server which will also provide acknowledgment. However, the absence of acknowledgment could indicate
that the LoRaWAN settings are not correct per the above.
