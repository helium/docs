...
You need the latest KerOS version (see above) and the appropriate Helium Gateway software installed:
...
Use the built-in tool `klk_apps_config` to activate the Kerlink Common Packet Forwarder (CPF) on the
appropriate region:
...
By default, Kerlink restricts the TX frequencies (downlinks) to what LoRaWAN&trade; defines.
Helium's Proof-of-Coverage uses the uplink bands for the transmission of the challenge RF packet.
...
- that the packet forwarder communicates well with the Helium Gateway because all the heartbeats /
  uplinks are acknowledged in a millisecond;
