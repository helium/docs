...
- **LoRaWAN&trade; Concentrator:** The concentrator module and Pi HAT to interface it with the
  Raspberry Pi. The following table contains the links to concentrators and their HATs.
...
LoRaWAN&trade; messages. To get started, install an operating system and set up the packet forwarder
as outlined in one of the two options below.

LoRaWAN&trade; gateways are generally 'headless' computers. The
