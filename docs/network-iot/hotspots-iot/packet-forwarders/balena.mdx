...
Use the Helium CLI Wallet to add this Hotspot to the network. The Helium Wallet App onboards
Hotspots over Bluetooth and requires an approved maker, so it cannot onboard a self-built Hotspot
like this one.

Follow [On-Device Onboarding](/iot/onboard-a-hotspot/on-device) to issue the Hotspot on-chain and
assert its location.
...
Once you have a device registered with an LNS, inspect packets from that device in your LNS to
confirm that this Hotspot is the one relaying the data. You will also be able to see in the
balenaCloud logs that UPLINK messages have been relayed. See
[Find a LoRaWAN Network Server Provider](/iot/find-a-lns-provider) if you do not have an LNS yet.
