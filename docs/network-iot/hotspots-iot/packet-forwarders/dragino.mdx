...
To be able to send and receive LoRaWAN data, your Gateway must be configured to use the frequency
plan specified for your region. You may be able to use the predefined frequencies in the Dragino
...
for your region as a guide to the values to use. You can check the values used by the predefined
...
<img
  ...
  src={useBaseUrl('img/iot-data-only/dragino/dragino-guide-lora-config-US915.png')}
/>
...
2. Select `Custom / Private LoRaWAN` as the Service Provider.
...
<img
  className="docsheader"
  src={useBaseUrl('img/iot-data-only/dragino/dragino-guide-lorawan-server-settings.png')}
/>
...
2. Copy the Helium Gateway package to the Dragino. The latest
   ...
   Option 2: Copy from another computer: Run this command on the computer within the directory
...
4. Back up your key file `/etc/helium_gateway/gateway_key.bin` and keep it in a safe place. This
   will be needed to recover this Gateway, especially if it is onboarded to the Helium IoT Network.
...
or rebooting the Dragino, as the messages may have rotated out of the log.
...
getting replies from the service as expected. Note that this is not a foolproof way to confirm it is
