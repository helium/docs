...
To prepare the hardware to become a Helium Hotspot, it must first be configured to handle LoRaWAN
...
If using Wi-Fi for initial setup, it is recommended to configure the Raspberry Pi using
...
<figure className="screensnippet-wrapper">
  <img
    src={useBaseUrl(
      'img/iot-data-only/rak-concentrators/rak-concentrators-guide-gateway-config-interface-region-plan.png',
    )}
...
packets to the address and port the helium_gateway service is listening on.
...
<figure className="screensnippet-wrapper">
  <img
    src={useBaseUrl(
      'img/iot-data-only/rak-concentrators/rak-concentrators-guide-packet-forwarder-settings.png',
    )}
    style={{ maxHeight: '450px' }}
...
Close the editor by pressing CTRL + X simultaneously, then press ENTER to save the file.
...
[the latest release](https://github.com/helium/gateway-rs/releases). You can download it to any
directory, for example `/tmp`.

#### Option 1

Download it from the repo directly to the Raspberry Pi by running the following commands:
...
A clean Raspberry Pi OS install may not have enough available RAM or swap space to build gateway-rs.
[Increase the swap space of the Pi](https://pimylifeup.com/raspberry-pi-swap-file/) for a smooth
build.
...
`settings.toml` file that exists within the `gateway-rs/config/` folder.
...
If there are errors in running the service, first stop the service
(`systemctl stop gateway-rs.service`), then edit it
...
The service starts automatically after installation. You can check the execution logs by running the
following command:
...
Network. It's highly advised to back up this file, especially if you need to recover it after the
onboarding process.

Using SCP is one option for the backup; that would look something like this:
...
Refer to the [on-device onboarding guide](/iot/onboard-a-hotspot/on-device) to complete the
