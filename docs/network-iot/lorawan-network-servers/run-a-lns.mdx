...
2. Open configuration/chirpstack/chirpstack.toml
   1. Change `net_id` to `0x00003C`
   2. Update the `dev_addr_prefixes` with the devAddr slab assigned to you. The format is
      `dev_addr _prefixes` = ["`starting devAddr`/`32-log2(devAddr slab size)`"]. Let's say you
      requested 8 devAddr slab and the starting devAddr is `00000000`, then you would input
