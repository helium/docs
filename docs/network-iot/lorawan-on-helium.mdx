...
an interface for users to configure routing rules relied on by HPR when determining where to forward
messages from end devices to LNSs. Users can leverage tools including the
...
send and receive packets to an end device, a network user needs to be serviced by an OUI.

An OUI has some specifics related to LoRaWAN and packet routing, as
...
From the gateway's perspective, the (AppEUI, DevEUI) may be used to figure out if one or more OUIs
...
filters themselves are _xorhash_ tables, so the AppEUI and DevEUI cannot be extracted. Only with the
AppEUI and DevEUI in hand can the gateway run them through the table to see if they're there.
