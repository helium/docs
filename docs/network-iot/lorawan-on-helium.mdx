...
Decentralized LNS Operation
In the Helium Network, the goal is to enable the independent operation of LNSs, moving away from a 
centralized model. This decentralization not only allows hotspots (gateways) to be independently 
owned and operated but also enables the independent operation of LNSs on the same public LoRaWAN 
Network. This approach distinguishes the Helium Network from other public wireless infrastructures 
by making it compatible with privately-run Network Servers.
...
- DevAddrs: These reserve NwkAddr space for the OUI's devices on the Helium Network. A device 
defined by (AppEUI, DevEUI) can be allocated to any of the DevAddrs owned by the OUI. It's even 
possible to multiplex many devices on the same DevAddr simultaneously, using the Message Integrity 
Check (MIC) to disambiguate.
...
- Blockchain Registration: Only libp2p addresses registered as endpoints for the OUI may open and 
close state channels on behalf of an OUI. This ensures secure and authorized operations within the
 network.
- Benchmarking and Testing: The upper limit for a Devaddr 1024 slab is approximately 5800 devices, 
based on benchmarking tests. However, community members are encouraged to run their own benchmarking
 tests to validate these results.
...
The process of joining the network involves a Join Request frame, which is unencrypted and contains
the AppEUI, DevEUI, and DevNonce. Gateways use this information to determine if any OUIs are
interested in the Join Request by querying filters maintained and written to the blockchain by every
OUI.
...
Data packets have an unencrypted header (FHDR) that includes the DevAddr, FCtrl, FCnt, and FOpts.
The DevAddr contains the NwkAddr of a device, which is allocated by the OUI's DevAddr slab. Gateways
can use the DevAddr to determine the OUI of Data frames, facilitating quicker look-up and routing.
...
The migration to the Solana blockchain has brought significant changes to the Helium Network,
particularly in how LoRaWAN Network Servers operate and how OUIs are managed. This update aims to
provide a clear understanding of these changes and their implications for network participants. For
those interested in running a Network Server or participating in the network, it's essential to stay
updated with the latest developments and guidelines provided by the Helium Foundation and the
community.
