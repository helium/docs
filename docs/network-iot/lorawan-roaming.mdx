...
- A Net ID issued by the LoRaWAN Alliance.
- A roaming endpoint so your LNS can receive roaming traffic.

You can refer to [this](/iot/run-an-lns/buy-an-oui) and [this](/iot/run-an-lns/fund-an-oui) to buy
and fund an OUI.

## Roaming Quickstart

While your roaming endpoint is up and running, you need to let Config Service know of its existence.
...
```bash
$ helium-config-service-cli route update http --dedupte-timeout <dedupte timeout> --path <roaming endpoint sub-path> --auth-header <auth header> --receiver-nsid <ReceiverNSID> --route-id <route-id> --commit
```
...
Session key filter gives you the ability to essentially accept and reject roaming traffic on a
device level. You can read more about that [here](/iot/run-an-lns/advanced-configuration)
...
diverse need of roaming partners.
