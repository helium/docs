...
With its global reach, the Helium Network sees a large amount of traffic from other wireless
networks. However, despite the fact this traffic is from compatible LoRa devices, it's dropped
because those devices have not been onboarded to the Helium Network.

This is where roaming comes in. Roaming on the Helium Network most closely resembles what is known
as “passive roaming”. A network operator can onboard their NetID, allowing devices and sensors to
send and receive traffic through a Helium Hotspot without requiring any device configurations or
firmware updates.
