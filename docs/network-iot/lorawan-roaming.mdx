...
description:
  Guide for configuring LoRaWAN roaming on Helium using the Config Service CLI to create routes and
  add EUI pairs.
...
- A Helium Network Organizationally Unique Identifier (OUI).
...
Once your roaming endpoint is up and running, you need to let the Helium Config Service know of its
existence. Initializing some CLI environment variables will streamline the process:
...
1.  You can then see the empty route template. It should look like this, except that you should see
    your NetID, assigned OUI, and max_copies value:
...
requests for your device, you need to let Helium know which EUI pair to watch out for. You can run
the following:
...
While this allows Helium to forward your device's join request, you also need to specify a devAddr
range on that route so that Helium can forward its uplinks.
...
have the same device simultaneously send traffic to both Roaming Console and OpenLNS Architecture.

#### Migrating EUI Pair for Join Requests

Let's say you have 100 devices on Roaming Console. You can start the migration slowly by registering
...
devices will roam through OpenLNS.
...
The Session Key Filter gives an operator the ability to accept or reject specific roaming traffic at
the device level. Leveraging the Session Key Filter is recommended for all operators. Read more
...
should provide a standard roaming interface. Note that Helium does not actively maintain this
