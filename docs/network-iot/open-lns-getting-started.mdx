---
id: open-lns-configuration
title: Getting started with Open LNS
pagination_label: Run and LNS
sidebar_label: Run an LNS
description: Helium Open LNS Documentation
image: https://docs.helium.com/img/link-image.png
slug: /lorawan-on-helium/open-lns-configuration
---
...
- LoRaWAN Network Server (LNS). A popular option is [ChirpStack](https://www.chirpstack.io). Refer
  to the ChirpStack documentation for setup and integration instructions.
...
- Fill in [`http://mainnet-config.helium.io:6080/`](http://mainnet-config.helium.io:6080/) for
  `Config Service Host`
- Fill in the path `./keypair.bin` for `Keypair Location`
- Fill in the Helium NetID <!-- TODO: get new NetID -->
...
Then export the environment variables. <!-- TODO: could use a little more detail -->

### Managing Routes

You are now ready to generate your first route. Routes contain necessary information to let the IoT
network know how to route a packet; DevEUI+AppEUI (for routing join requests), DevAddr range (for
routing uplinks), endpoint (where to send the packet to), etc.
...
Define DevAddr range so packet router knows which uplinks to route to your endpoint. Use your OUI's
assigned DevAddr range, which you can get by running `./helium-config-service-cli org get`
...
Next, define EUI pairs so packet router knows which join requests to route to your endpoint.
