...
the same device uplink within the Helium Network. LNS operators can customize the number of duplicate
uplinks they wish to receive and pay for by setting a max copies amount. Configuring the max
copies setting offers two significant potential advantages for receiving identical uplinks from multiple Hotspots:
...
route-level max copies setting of 2 and an SKF level max copies setting of 5 for device B. Then device A
will inherit the route level max copies setting = 2, and device B will use its SKF-level max copies
setting = 5.
...
max copies setting of 1. This will have the effect of allowing up to 20 copies of join requests and 1
