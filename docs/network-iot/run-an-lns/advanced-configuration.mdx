...
before buying it. Packets that fail the check are dropped before purchase. They never reach the LNS
and incur no Data Credit cost.
...
In practice, SKFs are kept in sync programmatically by the LNS. See the tools in
