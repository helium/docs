...
SKFs matter because devAddrs are not globally unique. Every LoRaWAN network assigns devAddrs from
the same 32-bit space, so a device on an entirely unrelated network can transmit on a devAddr that
falls in a Helium route. Without an SKF, the Packet Router cannot verify such packets and delivers
them anyway, charging the DC cost to the OUI owner.
...
Documentation on getting started can be found
[here](https://ccall48.github.io/chirpstack-hpr.github.io/).
