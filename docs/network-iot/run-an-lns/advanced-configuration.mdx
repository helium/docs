...
- If you submitted a session key on devaddr 1 to the config service, then HPR will only let traffic
  through on devaddr 1 if it can be MIC checked with one of the session keys
- If you did not submit a session key on devaddr 2 and the flag "ignore empty skf" = false, then HPR
  will let all traffic through on devaddr 2
- If you did not submit a session key on devaddr 2 and the flag "ignore empty skf" = true, then HPR
  will block all traffic on devaddr 2
