...
Each LoRaWAN Network Server (LNS) on the Helium Network is managed by means of an Organizationally
Unique Identifier (OUI). This registers the LNS with the blockchain and allocates two very important
...
Currently the purchasing of OUIs must be done by contacting the Helium Foundation. In the future it
will be self service.

DevAddr are sold in sequential blocks between 8 and 65,536 and any power of two.

You must purchase a slab when purchasing an OUI, therefore, the total minimum cost is US$900 : $100
for the OUI itself and $800 for eight DevAddr's.
