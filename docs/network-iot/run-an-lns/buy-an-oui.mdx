...
A device, defined by `(AppEui, DevEui)`, can be allocated any of the [DevAddrs](#devaddr-explained) owned by the OUI. It
...
Currently the purchasing of OUIs must be done by contacting the Helium Foundation. In the future
it will be self service.

DevAddr are sold in sequential blocks between 8 and 65,536 and any power of two.

You must purchase a slab when purchasing an OUI, therefore, the total minimum cost is US$900
...
Note that an OUI is only ever charged when a packet is received and properly
identified.
