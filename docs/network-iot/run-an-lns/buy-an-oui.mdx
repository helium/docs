...
As an alternate to purchasing a devAddr slab, it is also an option to retain a netID from the LoRa
alliance and use it in a roaming configuration. An OUI is required in either scenario.
...
Run this command twice. First for the owner keypair and second for the delegate keypair. Name files
accordingly.
