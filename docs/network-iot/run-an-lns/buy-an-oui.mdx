...
Currently, OUIs and devAddrs must be purchased by contacting the Helium Foundation. In the future,
these registrations will be self-service.

As an alternative to purchasing a devAddr slab, it is also possible to obtain a NetID from the LoRa
...
devAddrs are issued in blocks of 8 and cost \$12.50 per devAddr for a total of \$100. devAddrs are
...
generate the key pairs used for the OUI. Run the following commands to pull and build the Config
Service CLI:
...
Direct purchase of OUIs and devAddrs is not currently available on-chain. Send an email to
[iot@helium.com](mailto:iot@helium.com) containing the following information:

- The Helium wallet address
- The public key of your owner key pair
- The public key of your delegate key pair
- The number of devAddrs you want or your non-Helium NetID
...
ChirpStack randomly assigns a devAddr at join or re-join to the device from the available range and
then multiplexes each devAddr as needed. The ChirpStack platform does this through the assignment of
...
The computational overhead of these MIC checks is not heavy in most scenarios, as these checks are
designed to be performed on low-power devices. However, in high-traffic scenarios, it may be best to
allocate a larger set of devAddrs rather than additional compute.
...
devices are onboarded. The need for more devAddrs depends on a few factors:
