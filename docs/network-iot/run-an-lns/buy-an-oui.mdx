---
id: buy-an-oui
title: Buy An Organizationally Unique Identifier (OUI)
...
Each LoRaWAN&trade; Network Server (LNS) on the Helium Network is managed using an Organizationally Unique
...
the OUI. DCs are consumed in proportion to the network traffic transferred[^1]. If the DC balance
...
generate the key pairs used for the OUI. Run the following commands to pull & build the Helium Config
Service CLI:
...
Direct purchase of OUIs and DevAddrs are not currently available on-chain. Send an email with the subject line
'Purchasing OUI' to the [Helium Foundation](mailto:hello@helium.foundation) containing the following information:
...
Multiplexing in LoRaWAN&trade;, specifically DevAddr multiplexing, refers to the process where multiple
devices share the same Device Address (DevAddr). It's a way to manage the limited available range of
DevAddrs and allows more devices to join a network.
