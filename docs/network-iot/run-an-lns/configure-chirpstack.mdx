...
Now that you have your LNS up and running, the last step is to configure routing rules, so that the
network knows which LNS to send device packets too. Learn how to do that
[here](configure-routing-rules).
