...
(LNS) that provides a web interface and gRPC API for managing devices, routing messages, and
...
This guide describes running ChirpStack in a non-production setting. As such, the infrastructure
requirements noted below are sufficient for getting a ChirpStack instance up and running for
experimentational purposes, but are not suggested for use in a mission critical, production setting.
...
- Simlarly, the `chirpstack-gateway-bridge` container for the US915 region binds to UDP `1701`. As
...
Next, using the the `start_addr` and `end_addr` values, identify the `DevAddr` subnet masks, (e.g.,
...
To do so, ammend the `chirpstack-gateway-bridge` label to be `chirpstack-gateway-bridge-eu868` then
...
The ChirpStack web UI will now be accessible at `http://<public_ip_address>:8080`. Utilize the
