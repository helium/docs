...
## Initalize Helium Config Service CLI
...
```sh title="Setting environment variables as shown in a terminal window"
...
To add a device in Chirpstack, please follow the documentation in
[Add Device to ChirpStack](/iot/run-an-lns/chirpstack-add-device) and return here when complete.
...
the `--dev-eui` flag corresponds to the device EUI generated in ChirpStack, and the `--app-eui` flag
corresponds to the join EUI also generated in ChirpStack.

Please refer to the red-boxed portion of image below to find the device and join EUIs in ChirpStack.

<img className="docsheader" src={useBaseUrl('/img/open-lns/chirpstack-device-and-join-eui.png')} />
<br />
<br />
