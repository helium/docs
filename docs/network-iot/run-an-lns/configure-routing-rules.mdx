...
description:
  Using the Helium Config Service CLI to create routes, add device EUIs and DevAddrs, and register
...
Helium Config Service to enable sensor data to be routed to the ChirpStack instance.
...
1. For `Config Service Host`, press Enter to use the default value (i.e.,
...
5. For `Default Max Copies`, press Enter to use the default value (i.e., `15`).

After providing the initialization information, output similar to the following will be displayed:
...
Second, as suggested in the output, export the environment variables into the terminal session. This
needs to be done manually. To do so, add `export` to each line as shown below and run the commands:
...
Routes contain the necessary information for the Helium Packet Router to know how to route a packet
sent from a sensor to the proper LNS. In particular, Helium Packet Router leverages information
provided in a generated route such as DevEUI+AppEUI (for routing join requests), devAddr range (for
routing uplinks), and LNS endpoint to inform routing decisions.
...
In such cases, the command will show what would have been transmitted to the Helium Config Service
(i.e., a configuration service which informs routing behavior of the Helium Packet Router). In this
way, you can review the route information before formally submitting it.
...
Once the result looks as desired, simply re-run the command with the `--commit` flag to update the
route.
...
In preparing the command, the LoRaWAN region is followed by the port to be used for the region. In
the example command below, the region `us915` is followed by `<us915 port>`. Here, the
`<us915 port>` value will mirror the port bound to the `chirpstack-gateway-bridge-us915` container
(e.g., `1701`).

Please note that the command will also need to be run for the `eu868` region with port `1700`.
...
associate the device EUIs and devAddrs with the route.
...
the `--dev-eui` flag corresponds to the device EUI entered in ChirpStack, and the `--app-eui` flag
corresponds to the join EUI also entered in ChirpStack.

Please refer to the red-boxed portion of the image below to find the device and join EUIs in
ChirpStack.
...
Once the result looks as desired, simply re-run the command with the `--commit` flag to add the
device to the route.
...
[Gather Helium DevAddr Subnet Information for OUI](/iot/run-an-lns/docker-compose/#gather-helium-devaddr-subnet-information-for-oui).

```bash
helium-config-service-cli route devaddrs add --start-addr <start devAddr> --end-addr <end devAddr> --route-id <route-id>
```

Again, once the result looks as desired, simply re-run the command with the `--commit` flag to add
the devAddr to the route.
