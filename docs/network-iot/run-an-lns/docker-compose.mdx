...
description:
  Deploy ChirpStack on Docker Compose for non-production use, including configuration for Helium
  IoT.
...
running for experimentation purposes, but are not recommended for use in a mission-critical,
production setting.
...
server running the ChirpStack instance. In AWS parlance, a Security Group needs to be attached to
...
Next, using the `start_addr` and `end_addr` values, identify the `DevAddr` subnet masks (labeled as
`subnets` in the returned JSON). If there are multiple, non-contiguous `devaddr_constraints`, this
will need to be done for each set of constraints.
...
- Under `[network]`:
  ...
  - Add `dev_addr_prefixes` with the value `[<Subnet Mask>]`, e.g.,
    ...
    - If the OUI has multiple non-sequential `DevAddr` ranges, separate the corresponding subnet
      masks with commas. For instance, `dev_addr_prefixes=["00000000/29", "00000010/29"]`.
...
note that these values are examples and need to be filled in with the correct information for your
ChirpStack configuration. Again, all other sections in `chirpstack.toml` can be left as is.
...
the time of writing this guide, the current version of `chirpstack` was `4.7` and
`chirpstack-gateway-bridge` was `4.0`, so these versions will be used moving forward.
...
above in the `chirpstack.toml` file, add `chirpstack-gateway-bridge` service definitions for each
...
for a full description of the configuration possibilities for the `chirpstack-gateway-bridge`
service.

An example of a complete `docker-compose.yml` file is provided below:
...
The ChirpStack web console will now be accessible at `http://<public_ip_address>:8080`. Use the
