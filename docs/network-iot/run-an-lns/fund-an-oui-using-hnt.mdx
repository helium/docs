...
1. Select `HNT` in the top drop-down list and enter an amount of HNT to swap.
1. Select `DC` in the lower drop-down list and it will be filled with the resulting DC amount.
1. Tap the `Swap Tokens` button and confirm the swap transaction.
1. HNT is burned for DC within the same Wallet.

The new DC balance amount can be verified in the tokens list on the `$` tab within the Helium Wallet
app.
...
[ChirpStack Configuration](/iot/run-an-lns/configure-chirpstack) guides.
