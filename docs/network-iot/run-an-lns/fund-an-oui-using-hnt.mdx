...
The `price` command is a helpful tool for determing the amount of HNT required for a particular
amount of DC. For example, this command will output the amount of HNT required to fund 1M DC ($10).

```sh
helium-wallet dc price 1000000
```
...
```sh
helium-wallet dc mint --payee <Account Address> --dc 1000000
```

After the DC is minted, it can be delegated to the OUI. The delegation is issued to the router key
of the OUI.

```sh
helium-wallet dc delegate iot <Router Key> 1000000 --commit
```
...
1. Enter the XX key of the OUI as the address.
