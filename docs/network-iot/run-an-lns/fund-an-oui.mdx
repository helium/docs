---
id: fund-an-oui
title: Fund An Organizationally Unique Identifier (OUI)
...
[Data Credits](/tokens/data-credit) (DC) are used to pay for all transmission of data from devices
...
- [Fund an OUI using a credit card with the official Helium Data Credits Portal](/tokens/data-credit-portal)
...
Data Credits are charged per 1 DC per 24-byte increment for each response of an uplink payload. For
a helpful visualization of DC usage of devices for a given period, check out the
[Data Transfer calculator](/tokens/data-credit/#data-transfer-calculator) on the DC documentation.
...
charged 8 DC ($0.00008 USD). This assumes the payload was less than 24-byte.
...
The Helium Packet Router (HPR) enforces a minimum balance of 3,500,000 Data Credits ($35). This
minimum balance is required to ensure that all Hotspots are compensated for their work on the
Network in the case that the LNS transfers data more quickly than can be deducted from the OUI's
balance. Token balances cannot become negative, so this $35 buffer keeps network payments balanced.
