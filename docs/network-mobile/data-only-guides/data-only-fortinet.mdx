...
5. Add Helium Mobile and T-Mobile's NAI Realms.  
   Note: adding T-Mobile's NAI Realms prior to approval/communication makes the Access Request from
   T-Mobile's subscribers to be rejected at the Helium end, causing unnecessary signaling load.
   _Only add after Carrier Approval and Helium activation._

```shell
config wireless-controller hotspot20 anqp-nai-realm
    edit "Helium_TMO_NAI_Realm"
        config nai-list
            edit "freedomfi.com"
            ...
            next
            edit "wlan.mnc260.mcc310.3gppnetwork.org"
                set nai-realm "wlan.mnc260.mcc310.3gppnetwork.org"
                config eap-method
                    edit 1
                        set method eap-aka
                    next
            ...
            next
            edit "wlan.mnc530.mcc312.3gppnetwork.org"
                set nai-realm "wlan.mnc530.mcc312.3gppnetwork.org"
                config eap-method
                    edit 1
                        set method eap-aka
                    next
            ...
            next
            edit "wlan.mnc120.mcc310.3gppnetwork.org"
                set nai-realm "wlan.mnc120.mcc310.3gppnetwork.org"
                config eap-method
                    edit 1
                        set method eap-aka
                    next
            ...
            next
            edit "wlan.mnc240.mcc310.3gppnetwork.org"
                set nai-realm "wlan.mnc240.mcc310.3gppnetwork.org"
                config eap-method
                    edit 1
                        set method eap-aka
                    next
...
6. Add AT&T's 3GPP Cellular Networks.  
   Note: adding AT&T's 3GPP Cellular Networks prior to approval/communication makes the Access
   Request from AT&T's subscribers to be rejected at the Helium end, causing unnecessary signaling
   load. _Only add after Carrier Approval and Helium activation._

```shell
config wireless-controller hotspot20 anqp-3gpp-cellular
    edit "Helium_ATT_3GPP"
        config mcc-mnc-list
            edit 1
                set mcc "310"
                set mnc "410"
...
7. Configure the Passpoint Profile by linking the previously added NAI Realms, 3GPP Cellular
...
config wireless-controller hotspot20 hs-profile
    edit "Helium"
        ...
        set nai-realm "Helium_TMO_NAI_Realm"
        set 3gpp-plmn "Helium_ATT_3GPP"
        set ip-addr-type "ipv4-single-NATed-private"
    next
end
```

8. As the final step, set a Virtual Access Point (VAP) by linking the previously added “Helium
