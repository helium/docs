...
Use this NAS-ID in the [Data-Only Onboarding](/mobile/data-only-onboarding) flow and return to this
guide after the network is onboarded and certificates have been delivered.
...
After retrieving onboarded certificates, configuring the network controller for Passpoint is a
two-part process. First, create the RADIUS profile, then apply the profile to a newly created WiFi
SSID called 'Helium'.
