...
sidebar_label: Catalyst 9800
description:
  Configure a Cisco Catalyst 9800 (IOS XE) controller with RADIUS, Passpoint, and Hotspot 2.0 for
  Helium Mobile.
...
- Cisco IOS XE system must be running
  [17.9 or later](https://documentation.meraki.com/General_Administration/Firmware_Upgrades/Cisco_Meraki_Firmware_FAQ).
...
- ACLs or firewalls allow the 9800 WLC Controller and Docker container to communicate over UDP on
  ports 1812 and 1813
...
To start, log in to your Cisco 9800 Dashboard in your browser. This will be our starting point for
...
2. Confirm RADIUS is selected in the left column and click **+Add** in the top left
3. Enter a **Name**, e.g. Helium_AAA
4. Enter a **Server Address** with the IP of the RadSecProxy you installed earlier.
5. Enter the **Key** and **Confirm Key** fields equal to "mysecret" (modify if you have modified it
...
5. Enter the NAS-ID shared with you by Helium
...
15. In the drop-down for **IPv4 Type** select **Double NAT Private** (or the option that
    appropriately describes your network)
...
6. Select the **VLAN/VLAN Group** configured for Helium users (NOTE: This is typically a Guest type
   network with access to the internet only)
