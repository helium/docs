---
id: helium-plus-aruba
title: Aruba Conversion Guide
pagination_label: Aruba Conversion Guide
sidebar_label: Aruba
description:
  Configure an Aruba Mobility Controller with RadSecProxy, Passpoint, and Hotspot 2.0 for Helium
  Mobile.
image: https://docs.helium.com/img/link-image.png
slug: /mobile/helium-plus-aruba
---
...
RADIUS messages used for user authentication and session accounting are transmitted unsecured and
over UDP by default. By directing these messages internally in your secure network to a RadSecProxy,
the UDP traffic is then converted to a TLS-protected TCP connection to the Helium Network core AAA
servers.
...
- The Intel-based machine has a private IP in your network reachable from your Aruba Mobility
  Controller
- ACLs or firewalls allow the Aruba Mobility Controller and Docker container to communicate over UDP
  on ports 1812 and 1813
- ACLs or firewalls allow the container/host to reach the internet on TCP ports 2083 and 3802\.

### Container Deployment

1. Unzip and untar the `radsec-proxy-main.tar.gz` file into the directory of your choice on the host
   machine. This will unpack the following items:
...
To start, log in to your Mobility Controller GUI in your browser. This will be our starting point
for all major steps below.
...
11. Click **Next**
12. Select the Default Role based on your network configuration. In this example we use an
...
5. Select **Heium_dot1_svg** from the drop-down menu next to **Server Group**
...
5. Enter "Helium_IP" for profile name, then select appropriate values for IPv4 and IPv6
   availability. If unsure, select "**private-double-nated**" and "**not-available**".
6. Click **Submit**
7. Click **Pending Changes** in the top right corner.
8. Click **Deploy Changes** in the pop up.
...
6. In the drop-down for NAI Realm EAP Method 1, choose "**eap-tls**" and below that click the **\+**
...
5. Enter "Helium_venue" in Profile Name, select the best-fitting Venue Group, e.g. "**business**",
   and the best-fitting Venue Type, e.g. "**assembly-restaurant**", and supply a Venue Name, which
   can be the street address of your venue, e.g. "**303 Elk Ave**"
6. Click **Submit**
7. Click **Pending Changes** in the top right corner.
8. Click **Deploy Changes** in the pop up.
...
8. Select the best-fitting **Venue Group Type**, e.g. **assembly**
9. Select the best-fitting **Venue Type**, e.g. **assembly-restaurant**
10. Check the box next to **RADIUS Chargeable User Identity(RFC4372)**
11. Check the box next to **RADIUS Location Data (RFC5580)**
12. When complete, the form should look like the screenshot above.
13. Click **Submit**
...
6. Select **Helium_hs20** from the drop-down menu next to **Hotspot 2.0 Profile** at the top
...
Follow logs from the container and look for Accepts or Rejects for any reason.

- No RADIUS when the phone is trying means:
  - Aruba setup is not forwarding or not forwarding the right ports
  - Network is blocking; check for ACLs or firewalls in the network path
  - Container host network is blocking it from being sent to the container; check for firewall
    services on the host
- RADIUS arrives but you are getting rejects means:
  - Make sure the NAS-ID has been onboarded to the Helium Network
...
Wireshark/tcpdump on host for ports 1812, 1813, 2083, 3802\.  
