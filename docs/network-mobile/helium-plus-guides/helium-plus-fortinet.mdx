...
sidebar_label: Fortinet
description:
  Configure a Fortinet FortiGate with RadSecProxy, NAI realms, and Hotspot 2.0 for Helium Mobile.
...
- The Intel-based machine has a private IP in your network reachable from your Fortinet FortiGate
  box.
- ACLs or firewalls allow the Fortinet FortiGate box and Docker container to communicate via UDP on
  ports 1812 and 1813.
...
1. Unzip and untar the [`Helium_RadSec_Docker.tar.gz`](https://github.com/novalabsxyz/radsec-proxy)
...
- Radsecproxy.conf - The radsecproxy config file is pre-populated to connect to Helium Network AAA
  servers
- docker-compose.yml - File to start and stop the container as a daemon.

Into the same directory, copy the three certificates obtained from Helium:
...
1. Add the local RadSecProxy as the RADIUS server by replacing \<radsecproxy-ip-addr\> with the
...
3. Define a Venue Name and configure its duple by replacing \<any-venue-name\> with the desired
   value.
...
6. Configure the Passpoint Profile by linking the previously added NAI Realms, 3GPP Cellular
   ...
   network, among other parameters.
