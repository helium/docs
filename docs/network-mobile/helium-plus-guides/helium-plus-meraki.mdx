---
id: helium-plus-meraki
title: Cisco Meraki RadSecProxy Conversion Guide
pagination_label: Cisco Meraki RadSecProxy Conversion Guide
sidebar_label: Meraki - RadSecProxy
description:
  Configure Cisco Meraki with RadSecProxy, Passpoint SSID, and Hotspot 2.0 for Helium Mobile.
image: https://docs.helium.com/img/link-image.png
slug: /mobile/helium-plus-meraki
---
...
- The Intel-based machine has a private IP in your network reachable from your Meraki Mobility
  Controller.
- ACLs or firewalls allow the Meraki Mobility Controller and Docker container to communicate over
  UDP on ports 1812 and 1813.
...
1. Unzip and untar the `Helium_RadSec_Docker.tar.gz` file into the directory of your choice on the
   ...
   2. `Radsecproxy.conf` - The radsecproxy config file is pre-populated to connect to Helium Network
      AAA servers
...
To start, log in to your Meraki Dashboard in your browser. This will be our starting point for all
major steps below.
...
other parameters with default values set. Leave any unmentioned parameter at its default value.
...
each group of APs and asks for it to be loaded on the AAA. That does not scale and there is no way
to load the client certificate and key Helium produces during onboarding into the Meraki RadSec
client. So for now the guide below uses our RadSecProxy container and plain RADIUS configuration on
the Meraki devices.

:::

1. Click **Edit Settings**
2. Under the **Security** heading, toggle the circle **Enterprise with**
...
6. Repeat those steps to add the same server to the RADIUS Accounting Servers list using
   **port 1813.**
   <figure className="screensnippet-wrapper">
     <img
...
9. Change the number 1 dropdown for **NAS ID** to **Custom** and enter the NAS-ID you got during
   onboarding
...
5. Choose the best fitting **Venue Type** from the dropdown
6. Choose **Chargeable public network** from the dropdown
7. In the domain list, enter both **FreedomFi.com** and **Hellohelium.com**
