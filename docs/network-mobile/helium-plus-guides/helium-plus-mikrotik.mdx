...
- For self-serve, a CLI wallet prepared for onboarding:
...
At the time of writing, RouterOS does not support RadSec natively. Download the
[RadSecProxy](https://github.com/novalabsxyz/radsec-proxy) container and run it in Docker.
...
Paste the MAC address of the WAN interface (usually ether1 by default) from the Interfaces menu, or
enter the NAS-ID provided during Helium Plus onboarding.
...
Assign the configuration profile to the desired Wi-Fi interface. Either click **New** (this will
create a virtual AP) or **Edit** an existing master interface.

Make sure to select the correct 5/2.4 interface according to the band defined in the configuration
profile.
