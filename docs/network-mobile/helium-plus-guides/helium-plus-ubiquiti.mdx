...
<img
  ...
  src={useBaseUrl('img/mobile-data-only/ubiquiti/ubiquiti-carrier-offload-header.png')}
/>
<br />
<br />
...
or higher, depending on the hardware release track.
...
apply it to a new Wi-Fi SSID named **Helium**.
...
Configure a TLS connection to the Helium Cloud AAA server (aka Radiator), which performs
Authentication, Authorization, and Accounting for the end customers. Enabling RADIUS communication
over TLS (RadSec) increases the level of security for authentication that is carried out across the
cloud network.
...
1. Check the **Interim Update Interval** box.
...
<figure className="screensnippet-wrapper">
  <img
    ...
    className="add-border-radius add-shadow add-shadow-margin"
  />
  <figcaption>Create a new Wi-Fi network.</figcaption>
</figure>
<br />
