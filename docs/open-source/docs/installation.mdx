...
Download latest software: External link:
[Node.js](https://nodejs.org/en/download) version >= 10.15.1

:::info

Windows Node.js installation you do not need to install the Compiler and
Chocolaty dependencies.
...
Download software: External link:
[Yarn](https://yarnpkg.com/getting-started/install) version >= 1.5

After you install Node.js, which will install Node.js and npm as a dependency
...
To upgrade the Yarn version to be >= 1.5 you must set each project directory to
version berry.
...
Then you can install Yarn which will download and compile all the project module
dependencies.
...
Each time you save files under your local repo the site will get rebuilt and the
pages refreshed on the fly, there is no need to keep restarting Yarn.
