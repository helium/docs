...
and API costs incurred from the data requests. For more information on accessing data from a
requester pays bucket, refer to
...
calls over time - and save money - is to use the `aws s3api list-objects-v2` command and specify the
...
zipped bytestreams. In order to 'unpack' this data, you'll first need the associated protobuf from
...
that as a big endian u32. Then use that u32 number as the number of subsequent bytes to read. Decode those
subsequent bytes using the associated proto definition. Repeat this process until you've read the
entire file.

Alternatively you can adapt
[this sample oracle observer repo](https://github.com/dewi-alliance/oracle-observer-sample) for your
use-case.
