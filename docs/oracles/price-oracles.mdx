...
The [Price Oracle](/blockchain/oracles) is one of many Oracles on the Helium Network. The Price
Oracle is required on-chain as the price of HNT is needed to calculate HNT to burn when minting Data
Credits (DC).
