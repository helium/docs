...
```mermaid
flowchart TB;
    hotspots(Hotspots)-->|Send Packes|loadBalancer(Load Balancer);
    ...
    packerRouters-->|Forward packets to appropriate LNS|lns;
    lns[LNS]-->|Routes for a given LNS|configService;
...
entropy(Entropy)-->beaconer;
entropy(Entropy)-->witness1;
entropy(Entropy)-->witness2;

beaconer(Beacon Hotspot)-->|Beacon|witness1(Witness Hotspot 1);
beaconer(Beacon Hotspot)-->|Beacon|witness2(Witness Hotspot 2);
...
beaconer(Beacon Hotspot)-->|Beacon Report|loadBalancer;
...
DC burned, active device count, and veHNT staked are submitted to the chain throw the proxy of
