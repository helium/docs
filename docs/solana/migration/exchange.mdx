...
The Helium Network is migrating to the Solana blockchain, and all Helium Network Tokens, HNT, IOT,
and MOBILE, will be SPL tokens.
...
can be used to migrate wallet(s) to Solana. Instead of sending all transactions to inflate Helium
state on Solana during the Migration, transactions will process as needed.

The Helium Wallet App requests the migration-service to get the needed transaction and submits them
to the Solana blockchain when the app is opened for the first time after the Migration is complete.
