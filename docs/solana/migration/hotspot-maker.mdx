...
Maker Wallets will need both `SOL` and `DC` for transactions post Migration.

:::

### Maker Wallet SOL

Maker Wallets will need `SOL` to pay transaction fees and for minting new merkle trees if creating
double the current amount of Hotspots in their existing fleet.
...
- For more insight to the amount of SOL needed, check out
  ...
  - Enter the number of Hotspots to mint in the `How many compressed NFTs do you want to store?`
    field.
  - The `Uncompressed equivalent` value is a good approximation, round up when in doubt.
...
> Minting 10,000 Hotspots would require ~1.134 SOL in this example.

### Maker Wallet DC

Maker Wallets will need `DC` to pay onboarding and assert fees. DC can be aquired by either
