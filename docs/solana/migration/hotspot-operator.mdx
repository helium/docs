...
The IOT Premine will reward Hotspots contributing to the Network within the 30 days preceding the
Solana Migration Snapshot.
...
The [migration-service program](TODO: add link) will use this Snapshot to map existing rewards and
Network state to the Solana blockchain.

:::

The premine allocates IOT tokens in the following proportion: Validators: 2.5 billion tokens,
distributed proportionately to the amount of HNT staked at the time of the Snapshot. Hotspots: 2.5
billion tokens, distributed to all active Hotspots at the time of the Snapshot.
...
- Witnessed and been valid/invalid witnesses.
- Beaconed and had valid/invalid witnesses.
- Transferred data.

A Hotpsot is not "Active" if:

- Beaconed with no valid/invalid witnesses, and
- Has not passed data
...
At the time of the Solana Migration, all 5G Hotspots are also LoRaWAN Hotspots and will be allocated
Premine Rewards under the same conditions as LoRaWAN Hotspots.
