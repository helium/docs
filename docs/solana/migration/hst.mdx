...
This technology ensures that when HNT is emitted for an epoch, the HST share of HNT emissions is subsequently distributed to HST holders. Further,
...
The architecture of the fanout wallet has a few important considerations:
   ...
   3. Staked HST tokens will appear in your wallet as an NFT Named "Staked HST". **DO NOT BURN THIS NFT OR YOU WILL LOSE YOUR HST**. You can freely transfer this NFT between wallets. HNT will flow to the wallet holding the receipt, and the wallet holding the receipt is the only wallet that can unstake to receive the HST. 
   3. HST tokens must be unstaked in order to separate out tokens. Once unstaked, you can transfer freely, and the new wallet(s) can re-stake here.
...
You can stake and unstake your HST below. This will prompt you with two transactions. The first will stake the HST. The second will setup a clockwork job for distributions with enough sol to last 50 years. You can connect any Solana compatible wallet following the Wallet Standard, including the Helium Wallet App.
