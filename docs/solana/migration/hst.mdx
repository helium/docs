...
The architecture of the fanout wallet has a few important considerations:
   ...
   2. HNT can be manually claimed at any time, regardless of whether the clockwork job runs. Solana blockchain going down will not affect this, though it may affect when an epoch ends. If clockwork misses a day, when it runs the next day you will receive HNT for all epochs missed. 
   ...
   3. HST tokens must be unstaked in order to separate out tokens. Once unstaked, you can transfer freely, and the new Wallet(s) can re-stake then.
