...
made availabe on a basis of 'Requestor Pays' meaning you, the requester, will pay for the data 
...
sub-prefixes, where `<date>` is in `YYYY-MM-DD` format, starting from Feburary 19th, 2023 onward. 
For example, PoC data produced each day is comprised within a sub-prefix of `YYYY-MM-DD` format, so 
data produced on Feburary 20th, 2023 will be under a `2023-02-20` sub-prefix, data produced on 
Feburary 21st, 2023 will be under a `2023-02-21` sub-prefix, and so on. Any data produced before 
Feburary 19th, 2023 is stored under the `historical` prefix. The data flowing into the buckets is 
occuring in near-real time. 
