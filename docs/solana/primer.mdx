...
On Solana, a Wallet is just an account with some SOL-token in it, owned by the system program. The owner of
...
All the RPCs used in the Solana Migration follow the Solana chain and will index the NFT data
