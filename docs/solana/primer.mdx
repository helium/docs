...
`getProgramAddress([Buffer.from("helium", "utf8)], helium_sub_daos.programId)` will always return
the same address. Better yet, the `helium_sub_daos` is able to sign transactions as if it held the
private key to this PDA.
...
All of the RPCs used in the Solana Migration follow the Solana chain and will index the NFT data that Core Developers created in Compression NFTs.
This is one reason the Helium Network is able to mint a million or more NFTs on Solana. 

You may need to run an RPC service to access public Solana clusters. Many services are free and others are paid. 
The Foundation uses Helius as their RPC Provider, and it’s useful to use a PRC provider that is familiar with compression. They will all support this at some point.
