...
The claim example in this section requires a connection to an RPC node
(provider) that understands compressed NFTs. This is an advanced feature that
is not yet supported by Solana's public RPC servers. **This example
will not work unless you use an appropriate RPC provider.**
