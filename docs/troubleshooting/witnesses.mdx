...
Witness lists used to be used to construct multi-hop challenges, but since the move to single-hop challenges (or beacons), it is no longer used and only exists for informational purposes only.
