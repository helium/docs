...
Follow the instructions to submit a Marketplace Integration or Decoder pull request to the Helium Console github [repository](https://github.com/helium/console) for consideration by the Helium Foundation:
...
## Enable Marketplace Functions (For Console Hosting Providers)

Console Hosting Providers can choose which Functions are enabled for users by making the following configuration settings. 
...
An Integration must be enabled for devices to transfer data through it and a Decoder must be enabled to decode packets. For example, if an existing Integration is disabled, any devices will no longer be able to transfer data via that Integration.
...
- To enable selected integrations: `ALLOWED_INTEGRATIONS=<integrations>`. For example, to enable the
...
Console Hosting Providers can choose which Decoder Functions are enabled for users by making the following configuration settings. 

Set ```ALLOWED_FUNCTIONS``` .env variable to toggle desired functions.

To enable all reviewed functions (default): ```ALLOWED_FUNCTIONS=all```.
To enable selected integrations: ```ALLOWED_FUNCTIONS=<functions>```. For example, to enable the following functions: CUSTOM, CAYENNE: ```ALLOWED_FUNCTIONS=custom,cayenne```
