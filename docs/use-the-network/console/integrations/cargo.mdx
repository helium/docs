...
Last, apply any optional Labels. For more information on Labels please visit the [Labels guide](../labels).
...
The `decoded` field is the preferred method for sending data to Cargo. This method requires the use of a [Function Decoder](../functions) on Console in order to decode the device payload before sending it to Cargo.  The accepted fields are listed below and can be located at any depth within the `decoded` field.
