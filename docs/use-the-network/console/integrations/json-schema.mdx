...
When receiving or transmitting data with an integration there is a consistent
...
If the **payload_raw** field is the base64 encoded value
`'__clear_downlink_queue__'` any downlinks queued for that device will be
removed.
