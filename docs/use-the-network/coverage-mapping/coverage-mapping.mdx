...
Coverage mapping is a crowd-sourced effort to build a true-signal coverage map of the Helium network
across the globe. By mapping real-world coverage, network users can understand where sensor
...
[Mappers Roadmap](/use-the-network/coverage-mapping/mappers-roadmap). The roadmap is a great way to
understand the full scope of the project and find an area to make your own contributions to the
project.
...
Helium Console accounts are funded with 10,000 DC. Beyond that, HNT can be converted to DC to fund
...
[Mappers Quickstart](/use-the-network/coverage-mapping/mappers-quickstart) guide focuses on the the
Dragino LGT-92, but the steps are effectively the same for any mapping device.
...
through the console. Learn more about the
...
integrations for [Cargo](https://docs.helium.com/use-the-network/console/integrations/cargo/) and
