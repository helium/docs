...
Once the above sketch is compiled and you have checked the port and loaded the Serial monitor you should see the Device EUI. Copy this value!
...
After making a [Helium Console](https://console.helium.com) login, you we'll need select a new device "+" and on the Arduino sketch copy into the correct location   the SECRET_APP_EUI\(lsb\), and SECRET_APP_KEY\(msb\), written exactly as you see them on the Helium Console.
...
Note: See the end of this tutorial for an easier but less flexible CayenneLPP Adafruit MQTT example
