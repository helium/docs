...
Finally, we will paste in the License Key we obtained above to the sketch.  Replace the values in license[4] with the key obtained from [resource.heltec.cn/search](http://resource.heltec.cn/search)
...
to speed up this process you can make the following change below the sketch.
