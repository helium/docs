...
<img
  src={useBaseUrl(
    "img/use-the-network/devices/ready-to-use/dragino-lds02-banner.jpg"
  )}
/>
...
Open the enclosure and add two AAA battery cells for power.
...
for free, this device is supported out of the box by

To add the LDS02 to  Cayenne, you do not need to set up a Decode script
...
Lets begin by creating a new custom Function decoder, by selecting `Functions`, then `(+)`.
...
You'll see the LED flash green while it sends a packet to the network.

You should see the uplink message with the decoded fields as shown
below.
