...
is a self-contained sensor with LoRaWAN, 2xAAA batteries, a magnetic reed proximity switch and
accompanying magnet. When the magnet is aligned to the sensor and closer than a centimeter, the
switch is closed. When the magnet is moved further, the switch is open.
...
unused.) Follow our [quick start for instructions](/use-the-network/console/adding-devices) to add a
...
Open the LDS02 enclosure and add two AAA battery cells for power. For the best wide-range
...
This device is supported directly by [myDevices Cayenne](https://cayenne.mydevices.com/cayenne) via
the included Integration in Helium's Console.
...
manufacture of this device, Dragino, is using their own encoding scheme to define which bytes hold
which piece of data. Below is a table taken from the manual showing the size and location of each
piece of data.

<img src={useBaseUrl('img/use-the-network/devices/ready-to-use/dragino/lds02-byte-packing.png')} />
