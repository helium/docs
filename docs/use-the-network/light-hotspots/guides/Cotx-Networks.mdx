...
3. `scp` a downloaded release package for the supported platform to the gateway. Copy the Helium Gateway package to the Cotx X3. Run this command from on the computer within the directory containing the package `scp helium-gateway-<version>-<platform>.deb <gateway>:/tmp/`.
4. Install the Helium Gateway package `dpkg --install /tmp/helium-gateway-<version>-<platform>.deb`.
