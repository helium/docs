# Cotx X3 LoRaWAN Gateway Setup

This guide provides instructions on how to configure a Cotx X3 LoRaWAN gateway to work with the Helium network.

:::warning

Cotx X3 gateway will only mine HNT through packet transfer, and is considered a non-PoC Light Hotspot. The ability to mine HNT with anything other than [Approved Hotspots](https://helium.com/mine) is not currently supported. Please join the [#gateway-development](https://discord.gg/helium) channel in the Helium Discord Server for the latest updates.

:::

## Prerequisites

Prior to configuring your Cotx X3 to work with the Helium network, please confirm that it is connected to the Internet and updated to the latest firmware. 
...
To be able to send and receive LoRa data, your gateway must be configured to the frequencies specified for your region. You may be able to use the predefined frequency in the Cotx X3 or some cases may need to manually set them per the frequencies used by Helium in your region.
...
## Install Helium Gateway (gateway-rs) on the Cotx X3
...
2. Connect to the Cotx X3 over SSH. 
3. Copy the Helium Gateway package to the Cotx X3. The latest [source code](https://github.com/helium/gateway-rs) and pre-built [releases](https://github.com/helium/gateway-rs/releases) are available on GitHub. Option 1: Download from GitHub: On the Cotx X3 run `cd /tmp` and `wget -O helium-gateway-<version>-raspi234.deb https://github.com/helium/gateway-rs/releases/download/<version>/helium-gateway-<version>-raspi234.deb`. Option 2: Copy from another computer: Run this command from on the computer within the directory containing the package `scp helium-gateway-<version>-raspi234.deb /tmp/`.
4. Install the Helium Gateway package `dpkg --install /tmp/helium-gateway-<version>-raspi234.deb`.
5. The default region of the gateway is `US915`, if your region is different you can set the right one in `/etc/helium_gateway/settings.toml`. Just add the following line : `region = "<region>"` . Possible values are : `US915| EU868 | EU433 | CN470 | CN779 | AU915 | AS923 | KR920 | IN865`.
...
Your Cotx X3 gateway should now be setup and forwarding LoRaWAN data traffic to the Helium Network.

### Through the System Logs

Use the Cotx X3 system log to verify that Helium Gateway has started. You can view the log in the web interface or by using logread on the commandline interface (e.g, `logread | grep helium_gateway`). In the logs, you should see messages indicating helium_gateway is starting. If you do not see these, try running the gateway service (`/etc/init.d/helium_gateway server`) or reboot the Cotx X3 as the messages may have rotated out of the log.
