...
slug: /use-the-network/light-hotspots/guides/dragino
---
...
Dragino Hotspots will only mine HNT through packet transfer, and is considered a non-PoC Light Hotspot. The ability to mine HNT with anything other than
[Approved Hotspots](https://helium.com/mine) is not currently supported. Please
join the [#gateway-development](https://discord.gg/helium) channel in the Helium Discord Server for
the latest updates.
...
- Update to the latest firmware from
  [Dragino](http://www.dragino.com/downloads/index.php?dir=LoRa_Gateway/). 
  Search for the model of your hotspot > Firmware > Release > and download the .bin file.
  As recommended by Dragino, leave "Preserve Settings" _unchecked_ when upgrading
  ...
  to gateway-rs (via SSH: `opkg update` and `opkg install ca-bundle`). Note, 
  the OpenVPN firmware may be used however port numbers may differ from what's 
  included here.
...
frequencies specified for your region. You may be able to use the predefined 
frequency in the Dragino or some cases may need to manually set them per the
frequencies used by Helium in your region.
...
3. Under Radio Settings, select your region in the Frequency Plan drop down and, 
   if applicable, the Frequency Sub Band. For example, in the US, select US915
   for the frequency plan and 2 for the sub band.
...
[Helium sample LoRa packet forwarder global.conf files](https://github.com/helium/sx1302_hal/tree/helium/hotspot/packet_forwarder) for your region as a 
...
Then, you will install the Helium Gateway service by choosing one of the following:

### If using latest firmware, via web interface
Refer to the user guide provided on the [Dragino wiki](https://wiki.dragino.com/index.php?title=Notes_for_Helium) for more detailed instructions.

1. Log into the Dragino gateway web interface AFTER upgrading the firmware. 
2. Navigate to LoraWAN > Helium IoT from the Menu.
3. Click Download_Helium Server button. Wait for the confirmation message.
4. Click on Install_Helium_Server button. Wait for the confirmation message.
5. Click Save & Apply.

Helium Gateway is now installed and should be running. Login via SSH and use the command
`helium_gateway key info` to view the public key address and animal name of your
gateway.
Remember to backup your key file `/etc/helium_gateway/keypair.bin` and keep in a safe
place. This will be needed to recover this gateway especially if it is onboarded to the blockchain.


### If using older firmware, via SSH access

This requires SSH access to the Dragino.
...
2. Copy the Helium Gateway package to the Dragino. The latest
   ...
   GitHub. 
   Option 1: Download from GitHub: On the Dragino run `cd /tmp` and
...
3. Install the Helium Gateway package
   `opkg install /tmp/helium-gateway-<version>-dragino.ipk`.
4. Backup your key file `/etc/helium_gateway/keypair.bin` and keep in a safe
...
Note, during the alpha period, the helium gateway service by default routes data
traffic to the [Staging Router/Console](https://staging-console.helium.wtf/).
...
Data Only Hotspots can be added to the blockchain, assert its location, and be seen on explorer.helium.com.
Then, in [Milestone 2](https://docs.helium.com/use-the-network/light-hotspots/), you will earn HNT for data transfers (but not for Proof of Coverage). See
[Data Only Hotspots](https://docs.helium.com/mine-hnt/data-only-hotspots) for
the onboarding procedure. This process will require using the Helium
[command-line interface (CLI) Wallet](https://github.com/helium/helium-wallet-rs)
to submit the onboarding and assert location transactions. You will need the public key address of your hotspot that you got earlier via SSH, after installing Helium Gateway (gateway-rs) on the Dragino.
