...
Open LNS alleviates the usage and access bottlenecks that stem from "Helium Console" as the only LNS
option and instead scales network access through a large and diverse set of LNS providers and 
software suites.

**Accelerated Device Joins:** <br/>
The re-engineered backend architecture removes the need to store device routing information on the 
Helium blockchain, dramatically reducing device join time.
