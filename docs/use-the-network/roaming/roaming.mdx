...
With over 630K Hotspots in 166 countries and growing, the network sees a massive amount of
...
- **Configure Network Integration**- Specify the home network server address and port, input roaming
  device join credentials, and define how many packets to purchase for devices.
...
a packet costs 1 DC which equals $.00001.

### Purchasing Data Credits

Obtaining Data Credits is the same as on the Helium Console focused on device management and
onboarding. Users can use a credit card (recommended method) and sent up a recurring payment or burn
...
## Sample JSON
Here are the sample JSON payloads that roaming partners can expect to receive on their roaming endpoint
