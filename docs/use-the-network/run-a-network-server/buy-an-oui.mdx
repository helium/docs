...
2. The Router **misidentify** the offer thinking that it is from `Device9` (as they both share
   ...
   2. If Router does not deem offers associated with `Device9` worthy of purchasing (e.g. maybe
      `Device9` was de-activated by user in Console, or maybe `Device9`'s Console Organisation ran
      out of [Data Credit](/use-the-network/console/data-credits/)) the offer will be rejected and
      data never delivered. This is a conflict with **non-happy path**.
