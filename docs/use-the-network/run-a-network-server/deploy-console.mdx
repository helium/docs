...
[running Console](/use-the-network/run-console). Go ahead and follow that guide
up to **Run it!**.
...
Router, Postgres, and Nginx. You might see `helium_router` crash a few times as
it waits for `helium_console` to start up and allow the socket connection
between the two.
