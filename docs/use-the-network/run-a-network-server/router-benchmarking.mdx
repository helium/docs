...
[Generate a Console API Key](/use-the-network/console/my-account/#console-api-keys), which requires
logging onto Console user interface (UI). Everything else will be done by command-line interface
(CLI).
...
[cost of adding data-only hotspots and asserting location](/mine-hnt/data-only-hotspots) of each.
...
[mainnet](/mine-hnt/validators/#mainnet) blockchain, pick a bogus location such as middle of a lake
or river. (While there could be legitimate _devices_ in a particular body of water, there is
...
and then confirm asserted locations via [blockchain API](/api/blockchain/introduction/):
...
your wallet; see [Data Credits](/use-the-network/console/data-credits/) within Console
documentation.
