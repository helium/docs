...
Before we being, you should have the following:
...
- have a fully sync'ed Miner or ETL instance to extract a snapshot from
...
AUTH_0_DOMAIN=console.us.auth0.com
AUTH0_BASE_URL=https://router-dev.us.auth0.com
DATABASE_DB=console
DATABASE_HOST=postgres
```

The auth0 fields are taken from your auth0 dashboard where you create an app for
Console:
...
The key point is `ROUTER_CONSOLE_SECRET` from this file, matches
`ROUTER_SECRETS` from `.env`. You can ignore these other fields for now, but we
can revisit them later.

### Modify some files before building

- in `/config/prod.exs`, comment out everything except for `use Mix.Config`
- in `/config/releases.exs`, update your host from `example.com` to `localhost`
- at to the bottom of `/assets/webpack.config.js`, follow the instructions

### Run it!

`docker-compose up` launches the three containers: Console, Router, and
Postgres. You might see `helium_router` crash a few times as it waits for
`helium_console` to start up and allow the socket connection between the two.
...
```
$ docker exec helium_router _build/default/rel/router/bin/router peer addr
...
```
$ helium-wallet oui update --oui 4 --nonce 1 --address 11xHXS5AgLyjYRCJ4ctcWcsMRULS8jro9Pb1GPaTG1neGk1dNcf --commit
```
...
Login to Console and create an organization and a device. If you ever need help
with Console as a user,
[much documentation exists here](/use-the-network/console). If you see a
successful Join Request, then you are successfully running your private LoRaWAN
Network Server. Congratulations!
