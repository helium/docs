...
Staked tokens cannot be moved to other wallets, neither can the NFT representing a stake be transfered 
to another wallet. Stakes can only be transfered from one position into another within the same account. 
Positions can only be transfered to positions with a greater or equal duration. This functionality is 
useful for consolidating positions. 
Landrush positions cannot be transfered into after the landrush period. Any veHNT transfered out of a 
Landrush position will lose the 3x multiplier 3x bonus.

If no other compatible positions exist in an account, the "Transfer" button will be disabled for a position.

You must undelegate both positions before transfering a stake between postions. 
Transfering to a new position requires the original position to be undelegated.
You can redelegate one or both positions after the split is complete.
...
delegation purposes.
When you select the extend stake period you are updating the period of lockup of the stake, 
you are not selecting additional time to add to the existing period.

You must undelegate before extending a stake. You can redelegate after the extend is complete.
