...
As a note, Validator positions will appear in Realms as one large stake. `hntyv...` is the Solana
mint address for HNT.
...
period. Vote weight declines linearly until release. This is effectively starting the cooldown at
the moment of staking.
...
losing the multiplier for the tokens being split or transfered off the position.

You must undelegate before spliting a stake. You can redelegate after the split is complete.

### Transfer a Stake

Neither staked tokens nor the NFT representing a stake can transfer from one wallet to another.
Stakes can only be transferred from one position to another within the same wallet. Positions can
only transfer to positions of greater or equal duration. This functionality is useful for
consolidating positions. Landrush positions cannot be transferred into after the landrush period.
Any veHNT transferred out of a Landrush position will lose the 3x multiplier 3x bonus.

If no other compatible positions exist in an account, the "Transfer" button will be disabled for a
position.

You must undelegate both positions before transfering a stake between postions. Transfering to a new
position requires the original position to be undelegated. You can redelegate one or both positions
...
period does not add additional time to the stake, instead updating the existing stake. For example,
a 2-year stake extended to 4 years results in a 4-year stake, not a 6-year stake.
...
transfers. In order to stake your HNT you will need approximately 0.006 SOL in your account in order
