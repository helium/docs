...
must delegate their veHNT to one or both subDAOs. The lock-up period can range from 1 day to 48
...
veTokens can be managed through Realms using the in-app browser on the
...
SubDAO rewards are calculated and paid to the lock-up position each 24hr reward period. Currently,
the IOT and MOBILE subDAO each have a 6% token emission pool for veHNT stakers.
