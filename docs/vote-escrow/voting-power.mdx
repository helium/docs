...
Improvement Proposals is performed with vote escrowed tokens.
...
- After the minimum lock-up period, HNT releases to the owner's wallet after the staking cooldown is
  selected.
...
Now the Solana migration has been completed, stakers can use their staked veHNT to vote on proposals in
...
your unstaking date at the time of lock-up or decide to keep it on a rolling lock-up period,
...
> (10x12.5x3) that, if a cooldown is selected, decays from 375 to 0 veHNT at the end of the
> staking period, and 10 HNT is returned to the wallet.
...
undelegate, change the period, redelegate, and lose delegation rewards within that 24 hour Epoch.
...
start point will be need positive action to start the lock-up cooldown period.
