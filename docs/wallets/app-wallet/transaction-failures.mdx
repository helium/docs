...
How much HNT to burn depends on the Oracle price. The Oracle price is the median $HNT price from a
list of individual oracles and used to weather large swings in price. In special cases, the price of
$HNT may shift enough in a short time period such that the amount of HNT burned to generate DCs _is
not enough_.
