...
## Transfer Hotspot from 24-words Wallet

If a Hotspot is transferred to a 24-words wallet, that is ok! However, most Maker Hotspot Management
Apps do not currently support 24-words wallets.

To transfer a Hotspot from a 24-words wallet to a 12-words wallet, follow the steps below.

1. Create a 12-words wallet to receive the Hotspot using the
...
3. Import your 24-words seed phrase to the CLI Wallet.

```bash
./helium-wallet create basic --seed bip39 --output my-24-words-wallet.key
```
...
| `--output my-24-words-wallet.key` | output the wallet key file to a location of your choosing, in this case to a file called `my-24-words-wallet.key` |
...
```bash
./helium-wallet -f my-24-words-wallet.key hotspots transfer <gateway> <new-owner>
```

| argument                    | description                                                              |
| --------------------------- | ------------------------------------------------------------------------ |
| `-f my-24-words-wallet.key` | use a file for the hotspot wallet, in this case `my-24-words-wallet.key` |
...
```bash
./helium-wallet -f my-24-words-wallet.key hotspots transfer <gateway> <new-owner> --commit
```
