...
import your key from the mobile wallet, so that you can submit transactions that may not be supported
in the mobile wallet. The `--seed` command supports both 12-word and 24-word imports.
