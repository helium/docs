...
This guide will walk through using Helium on a Ledger Nano S but is similar to setup on the Nano X.
...
[Releases in the Github repo](https://github.com/helium/helium-ledger-cli/releases) and download the
most recent release for your operating system.

We'll use macOS for the remainder of this example.

- Download the release for macOS `helium-ledger-cli-x.x.x-x86_64-apple-darwin.zip` and unzip the
  file
...
Your Ledger hardware wallet can actually support many accounts per supported coin and this is no
different for HNT. To interact with a wallet other than the default 0th account, you place a
`--account N` flag before the command. So the balance command becomes:
...
example: `helium-ledger-cli --account 8 balance --scan`
...
We will document an example `pay` transactions here, but the documents may lag changes in the app.
...
`--help` flag. For example: `./helium-ledger-cli --help`
...
practice for using the Ledger and making sure you understand the security model.
...
Use the screen of the Ledger to confirm everything you do. The CLI app could be nefarious (maybe you
downloaded the wrong one, maybe somebody change the program on your computer, etc). The Ledger on
the other hand will only load applications signed by Ledger, so your security relies on you
...
run a command again in the CLI.
...
ensure its own correctness and protect the user from the computer that the Ledger Hardware Wallet is
...
Example: the user runs `./helium-ledger-cli balance` to display their public key to so that they may
